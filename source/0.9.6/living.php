...
/* KiCom Living Architecture 0.9.6 */
...
function kicomTotpPublicStatus(): array {
    $cfg=kicomTotpConfig();$p=kicomTotpPending();return ['configured'=>$cfg!==null,'pending'=>$p!==null,'digits'=>6,'period'=>30,'algorithm'=>'SHA1'];
}
...
function kicomAutonomySessionConsume(string $id,string $token): array {
    ...
    $next=kicomAuthRandomHex(32);$r['token_hash']=hash('sha256',$next);$r['last_used_at']=gmdate('c');$r['idle_expires_at']=min((int)$r['absolute_expires_at'],$now+(int)$policy['idle_ttl']);
    if(!kicomAuthJsonWrite($file,$r))return ['ok'=>false,'code'=>'SESSION_ROTATE_FAILED'];return ['ok'=>true,'session_id'=>$id,'next_token'=>$next,'expires_at'=>(int)$r['absolute_expires_at'],'idle_expires_at'=>(int)$r['idle_expires_at']];
}
...
function kicomAutonomySourceRead(string $path,int $offset=0,int $length=3000): array {$path=kicomSafeUpdatePath($path);if($path===null||!in_array($path,kicomAutonomySourcePaths(),true))return ['ok'=>false,'code'=>'SOURCE_PATH_FORBIDDEN'];$f=kicomBaseDir().'/'.$path;if(!is_file($f))return ['ok'=>false,'code'=>'SOURCE_FILE_MISSING'];$size=(int)filesize($f);$offset=max(0,$offset);$length=max(1,min(3000,$length));if($offset>$size)return ['ok'=>false,'code'=>'SOURCE_OFFSET_INVALID'];$h=@fopen($f,'rb');if(!$h)return ['ok'=>false,'code'=>'SOURCE_READ_FAILED'];fseek($h,$offset);$raw=(string)fread($h,$length);fclose($h);$next=$offset+strlen($raw);return ['ok'=>true,'path'=>$path,'bytes'=>$size,'sha256'=>hash_file('sha256',$f)?:'','offset'=>$offset,'next_offset'=>$next,'eof'=>$next>=$size,'data'=>kicomEncodeBase64Url($raw)];}
