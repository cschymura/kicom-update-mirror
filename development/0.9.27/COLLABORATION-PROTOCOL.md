...
## Handwerker: Notfall-Zentralenrolle bei Kommunikations- oder Wissensausfall (2026-09-20)

- **Christophs Ziel: Ein Handwerker kann bei Ausfall der normalen Zentrale deren *Funktion* vorübergehend übernehmen.** Er wird dadurch nicht dieselbe ChatGPT-Unterhaltung, erhält kein verborgenes Gedächtnis, keine bereits nicht verfügbaren Tools, zusätzlichen Rechte oder fremde Identität. Er meldet `NOTFALL-ZENTRALE <eigene Kennung>` und dokumentiert den tatsächlichen Ausfallgrund.
- Rekonstruiere selbstständig den *aktuell nachweisbaren* gemeinsamen Projekt- und Auftragsstand über verfügbare, autorisierte Wege: zuerst KiCom BOOTSTRAP und lesbare Ressourcen, GitHub COLLABORATION-PROTOCOL und neuesten verifizierten Engram-Checkpoint, dann Slack-Jobthreads, soweit erreichbar. Bereits geladene Version, Zeitpunkt, Herkunft und Lücken notieren. Eine einzelne fehlgeschlagene Quelle darf nicht zum Abbruch führen, aber fehlende Aktualität ist explizit kenntlich zu machen.
- Bei erreichbarem gemeinsamem Kanal darf die Notfall-Zentrale offene und blockierte Jobs sichten, klar abgegrenzte neue oder korrigierende Jobs veröffentlichen, Doppelarbeit/Abhängigkeiten prüfen und nachweisbare Ergebnisse zur Übergabe bündeln. In Slack-Nachrichten niemals geheime Daten, privaten Engram-Inhalt oder vermeintlich neue Autorisierungen veröffentlichen.
- Wenn Slack ausfällt: GitHub nur über einen **tatsächlich verfügbaren autorisierten Schreibweg** und in einen eigenen konfliktfreien DEV-/Handoff-Pfad zur Ablage *öffentlichen technischen* Arbeitsstands nutzen; kein bestehendes kanonisches Dokument unkoordiniert überschreiben. Kein zentraler Schreibweg? Abgeschlossene rein lesende Analyse oder isolierte synthetische Arbeit an die aufrufende iOS-Aktion als `NOTFALLZENTRALE_OFFLINE_ERGEBNIS / NICHT_ZENTRAL_GESPEICHERT` zurückgeben; lokale Ablage nur dann behaupten, wenn die iOS-Automation sie tatsächlich speichert.
- Bestehen gleichzeitig mehrere Notfall-Zentralen, arbeiten sie nicht als exklusive Leitung: mit eigener Kennung und Stand prüfen, konkurrierende Koordination/Änderungen vermeiden, eine andere belegte aktive Leitung respektieren und isolierte read-only/synthetische Jobs bevorzugen. Keine Reservierung oder Führung allein aus gemeinsamem Slack-Absender oder Zeitstempel als atomar behaupten.
- Sobald der gemeinsame Kommunikations-/Wissensweg wieder verfügbar ist, dokumentierten Zwischenstand nachtragen und in die reguläre Auftragsverwaltung übergeben; Doppelmeldungen anhand eigener Job-/Ergebnis-IDs erkennen. Nur wirklich autorisierte Aktionen ausführen. Keine Freigabe-, Auth-, Installations-, Vertraulichkeits- oder Kostenrestriktion durch Notfallstatus aufheben.
