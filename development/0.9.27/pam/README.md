...
- KiComPamSnapshotSequencer.php: isolated append-only sequential ledger with file locking around a trusted native snapshot writer, per-entry SHA-bound identity and cross-restart verification. Detects out-of-band/legacy snapshots and interrupted publication without guessing order. **Not deployed; it cannot safely replace native recovery until every snapshot producer participates in the lock, orphan reconciliation and ledger anchoring are reviewed.**
- test-snapshot-order.php and test-sequencer.php: 11 conservative legacy selection tests and 12 append-only sequencing/restart/tamper/interrupted-publish tests. Current total: 32 core + 22 KCL + 13 persistence + 11 snapshot-order + 12 sequencer = 90 isolated tests. R3 compatibility CI adds native two-snapshot sequence tests (latest total 51).
...
7. Before any recovery-core change, inventory ALL native snapshot producers/callers (maintenance, evolution, manual operations, repair). Ensure a shared trusted lock protects every new snapshot through durable journal publication; reconcile legacy/unsequenced files without guessing order, and persist a high-water anchor outside a truncatable ledger. Prove the preflight runs *before* any destructive quarantine/restore. The existing R3 kicomSqliteLatestSnapshot() still sorts random-suffixed, second-resolution filenames and is not yet fixed.
