...
function kicomAutonomyUploadFinish(string $sessionId,string $id): array {
    ...
    elseif($kind==='self_update'){
        $tmp=kicomTempDir().'/autonomy-'.substr(hash('sha256',$raw),0,20).'.zip';if(@file_put_contents($tmp,$raw,LOCK_EX)===false)$result=['ok'=>false,'code'=>'PACKAGE_TEMP_WRITE_FAILED'];else{try{$source=(string)($meta['source']??'autonomy:session');$result=kicomReceiveSelfUpdatePackage($tmp,(string)($meta['filename']??'KiCom-upload.zip'),$source,true);if(!empty($result['ok'])&&($result['code']??'')==='STAGED_DECISION_REQUIRED'&&($result['risk_class']??($result['staged']['risk_class']??''))==='yellow'&&!empty(kicomAutonomyPolicy()['auto_yellow_from_session'])){$pending=kicomSelfUpdatePending();if(is_array($pending))$result=kicomApplySelfUpdate($pending)+['code'=>'AUTO_INSTALLED_YELLOW_SESSION'];}}finally{@unlink($tmp);}}
    }
