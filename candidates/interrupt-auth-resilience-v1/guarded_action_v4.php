...
/* Preferred normal-autonomy wrapper: exact replay + never-blind-reexecute +
 * request-bound previous-token fallback. */
function kicomGuardedNormalActionV4(string $sessionId,string $presentedToken,string $requestId,string $operation,array $params,callable $action): array {
    $g=kicomRequestGuardBeginV4($sessionId,$requestId,$operation,$params,$presentedToken);if(empty($g['ok']))return $g;$mode=(string)($g['mode']??'');
    ...
    $claim=(string)($g['claim']??'');$ss=kicomAutonomySessionConsumeResilientV3($sessionId,$presentedToken,$requestId);
    ...
    $done=kicomRequestGuardCompleteV4($sessionId,$requestId,$operation,$params,$presentedToken,$claim,$response);if(empty($done['ok']))return ['ok'=>false,'code'=>'REQUEST_REPLAY_PERSIST_FAILED','detail'=>(string)($done['code']??'UNKNOWN'),'action_response'=>$response,'inspect_required'=>true];return $response;
}
