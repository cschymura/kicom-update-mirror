...
// Retention is append-only: a legitimate prior snapshot may already exist.
// The gate must prove that THIS invocation adds exactly two new snapshots,
// rather than assuming the private backup directory was empty forever.
$prior=$back.'/dev95-retained-prior-mirage-oauth.sqlite';
if(!copy($data.'/mirage-oauth.sqlite',$prior))throw new RuntimeException('fixture prior backup');chmod($prior,0600);
$priorDb=new PDO('sqlite:'.$prior);ok100($priorDb->query('PRAGMA quick_check')->fetchColumn()==='ok','retained prior snapshot fixture valid');$priorDb=null;
$beforeSnapshots=glob($back.'/dev95-*.sqlite')?:[];sort($beforeSnapshots);
$pre=KiComEngramActiveSchemaUpgrade::preflight($web,$runtime);ok100(($pre['ok']??false)===true,'native merged preflight');
$bad=$runtime;$bad['enabled']=false;deny100(fn()=>KiComEngramActiveSchemaUpgrade::applyAfterVerifiedPasskey($web,$bad),'inactive runtime denied before DDL');$afterDenied=glob($back.'/dev95-*.sqlite')?:[];sort($afterDenied);ok100($afterDenied===$beforeSnapshots,'denied runtime creates no snapshots and preserves retained backups');
...
$afterSnapshots=glob($back.'/dev95-*.sqlite')?:[];sort($afterSnapshots);$copies=array_values(array_diff($afterSnapshots,$beforeSnapshots));ok100(count($copies)===2,'exactly two new native SQLite pre-DDL snapshots created despite retained backups');foreach($copies as $f){ok100((fileperms($f)&0077)===0,'new snapshot private');$s=new PDO('sqlite:'.$f);ok100($s->query('PRAGMA quick_check')->fetchColumn()==='ok','new snapshot quick_check');}
ok100(is_file($prior),'retained prior snapshot preserved');
