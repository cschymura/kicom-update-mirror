...
try{
 ...
 foreach(['ExpansionProtocol.php','CellNode.php','CellLiving.php','CellPerceptionAction.php'] as $f) chk(copy(__DIR__.'/'.$f,$src.'/'.$f),'copy source '.$f);
 ...
 $seed=require $pkg['directory'].'/bootstrap.config.php';$child=new KiComExpansionCellNode($pkg['directory'].'/var');$init=$child->initialize($seed);chk(!empty($init['ok']),'child init');chk(!empty($init['living_ready']),'living ready at birth');chk(!empty($init['perception_action_ready']),'perception/action ready at birth');$born=$child->status();chk(!empty($born['living_ready'])&&!empty($born['perception_action_ready']),'status proves complete birth before enrollment');
 $done=$o->activatePrepared($prep,new FakeTransport($child));chk(!empty($done['ok'])&&($done['code']??'')==='EXPANSION_ACTIVE','activation+tick');$s=$child->status();chk(($s['state']??'')==='active','child active');chk(!empty($s['living_ready'])&&!empty($s['perception_action_ready']),'child still living+PA ready');chk(count($reg->cells())===1,'registry cell');
