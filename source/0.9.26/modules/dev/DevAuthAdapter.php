...
if (!class_exists('KiComPasskeyBridge', false)) {
    $local = __DIR__.'/PasskeyBridge.php';
    $source = dirname(__DIR__).'/passkey-auth/PasskeyBridge.php';
    require_once is_file($local) ? $local : $source;
}
require_once __DIR__.'/DevSession.php';

/**
 * Binds successful passkey assertions to practical DEV sessions.
 * Verification and issuance are serialized per challenge, so a double tap or
 * concurrent request cannot mint two DEV sessions from one assertion challenge.
 */
final class KiComDevAuthAdapter
{
    ...
    public function __construct(KiComPasskeyBridge $passkeys,KiComDevSessionManager $sessions,string $lockDir)
    {
    ...
    public function verifyAndIssue(string $challengeId,array $credential,string $label='KiCom DEV'): array
    {
        ...
        if (!preg_match('/^[a-f0-9]{32}$/',$challengeId)) return ['ok'=>false,'code'=>'CHALLENGE_ID_INVALID'];
        $ready=$this->ready();
        ...
        try {
            ...
            return $session+['auth_method'=>'passkey','challenge_id'=>$challengeId];
        } finally {
            @flock($fh,LOCK_UN);
            @fclose($fh);
        }
