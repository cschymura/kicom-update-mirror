...
function kicomDelegatedBuildPlanHashV1(array $plan): string { return hash('sha256',kicomDelegatedBuildPlanCanonV1($plan)); }
function kicomDelegatedBuildPlanValidateV1(array $plan): array {
    ...
    $out=['build_id'=>$buildId,'steps'=>$clean];return ['ok'=>true,'plan'=>$out,'plan_sha256'=>kicomDelegatedBuildPlanHashV1($out)];
}
function kicomDelegatedBuildActiveCountV1(): int {
    if(!kicomDelegatedBuildJobsEnsureV1())return PHP_INT_MAX;$n=0;
    foreach(glob(kicomDelegatedBuildJobsDirV1().'/*.json')?:[] as $f){$r=kicomAuthJsonRead($f);if(is_array($r)&&in_array((string)($r['state']??''),['READY','RUNNING','UNCERTAIN'],true))$n++;}
    return $n;
}
...
function kicomDelegatedBuildFailV1(array $r,string $code): array {
    $r['state']='FAILED';$r['last_code']=$code;$r['updated_at']=gmdate('c');kicomAuthJsonWrite(kicomDelegatedBuildJobFileV1((string)$r['id']),$r);return ['ok'=>false,'code'=>$code];
}
function kicomDelegatedBuildJobTickV1(string $id): array {
    ...
    try{
        ...
        $s=$steps[$n];$r['state']='RUNNING';$r['last_code']='STEP_RUNNING';$r['updated_at']=gmdate('c');if(!kicomAuthJsonWrite(kicomDelegatedBuildJobFileV1($id),$r))return ['ok'=>false,'code'=>'JOB_WRITE_FAILED'];
        $x=kicomFastBuildPatch((string)$r['session_id'],(string)$plan['build_id'],(string)$s['path'],(string)$s['base_sha256'],(string)$s['find'],(string)$s['replace']);
...
function kicomDelegatedBuildJobTickAllV1(int $limit=4): array {
    ...
    foreach(glob(kicomDelegatedBuildJobsDirV1().'/*.json')?:[] as $f){if(count($rows)>=$limit)break;$id=basename($f,'.json');$r=kicomDelegatedBuildJobReadV1($id);if(!is_array($r)||(string)($r['state']??'')!=='READY')continue;$rows[]=['job_id'=>$id,'result'=>kicomDelegatedBuildJobTickV1($id)];}
