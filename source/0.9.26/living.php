...
/* KiCom Living Architecture 0.9.12 */
...
function kicomLivingEvent(string $type,string $severity='info',array $data=[]): void {if(!defined('KICOM_RECOVERY_EMBEDDED'))define('KICOM_RECOVERY_EMBEDDED',true);require_once __DIR__.'/recovery.php';rkEvent($type,$severity,$data);if(function_exists('kicomSqliteEventAppend'))kicomSqliteEventAppend('living',['at'=>gmdate('c'),'type'=>$type,'severity'=>$severity,'data'=>$data],null);}
...
function kicomLivingExperience(int $limit=500): array {kicomLivingEnsure();$rows=function_exists('kicomSqliteEvents')?array_reverse(kicomSqliteEvents('living',$limit)):[];if(!$rows){$f=kicomLivingEventsFile();if(is_file($f)){$lines=@file($f,FILE_IGNORE_NEW_LINES|FILE_SKIP_EMPTY_LINES)?:[];foreach(array_slice($lines,-$limit) as $line){$r=json_decode($line,true);if(is_array($r))$rows[]=$r;}}}$types=[];$paths=[];$severity=[];foreach($rows as $r){$t=(string)($r['type']??'unknown');$types[$t]=($types[$t]??0)+1;$sev=(string)($r['severity']??'info');$severity[$sev]=($severity[$sev]??0)+1;$d=$r['data']??[];if(is_array($d)){foreach(['path','component'] as $kk)if(is_string($d[$kk]??null)){$p=$d[$kk];$paths[$p]=($paths[$p]??0)+1;}foreach(['repaired','quarantined','files'] as $lk)foreach(($d[$lk]??[]) as $p)if(is_string($p))$paths[$p]=($paths[$p]??0)+1;}}arsort($types);arsort($paths);arsort($severity);return ['events'=>count($rows),'types'=>$types,'paths'=>$paths,'severity'=>$severity,'recent'=>array_slice(array_reverse($rows),0,20)];}
...
function kicomEvolutionAtomicJson(string $file,array $row): bool {$m=function_exists('kicomSqliteManagedFileKey')?kicomSqliteManagedFileKey($file):null;if(is_array($m))return kicomSqliteKvWrite((string)$m['namespace'],(string)$m['key'],$row,$file);$json=json_encode($row,JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);if($json===false)return false;$tmp=$file.'.tmp-'.substr(hash('sha256',uniqid('',true)),0,12);if(@file_put_contents($tmp,$json."\n",LOCK_EX)===false)return false;@chmod($tmp,0600);if(!@rename($tmp,$file)){@unlink($tmp);return false;}@chmod($file,0600);return true;}
function kicomEvolutionReadJson(string $file): ?array {$m=function_exists('kicomSqliteManagedFileKey')?kicomSqliteManagedFileKey($file):null;if(is_array($m))return kicomSqliteKvRead((string)$m['namespace'],(string)$m['key'],$file,[]);if(!is_file($file))return null;$r=json_decode((string)@file_get_contents($file),true);return is_array($r)?$r:null;}
...
function kicomGoalAppendEvent(string $file,array $row): void {$row=['at'=>gmdate('c')]+$row;if(function_exists('kicomSqliteEventAppend')){kicomSqliteEventAppend('goals',$row,$file);return;}$json=json_encode($row,JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);if($json!==false)@file_put_contents($file,$json."\n",FILE_APPEND|LOCK_EX);}
...
function kicomGoalRegistryRaw(): array {kicomGoalMemoryEnsure();$r=function_exists('kicomSqliteKvRead')?kicomSqliteKvRead('goals','registry',kicomGoalRegistryFile(),['schema'=>1,'policy'=>'archive-never-hard-delete','goals'=>[]]):null;if(!is_array($r)){$f=kicomGoalRegistryFile();$r=is_file($f)?json_decode((string)@file_get_contents($f),true):null;}if(!is_array($r))$r=['schema'=>1,'policy'=>'archive-never-hard-delete','goals'=>[]];if(!is_array($r['goals']??null))$r['goals']=[];return $r;}
...
function kicomTotpVerifyConsumeWindow(string $code,string $purpose,int $pastSteps=0): array {
...
function kicomAutonomyPolicy(): array {
    $defaults=['schema'=>1,'enabled'=>true,'session_ttl'=>28800,'idle_ttl'=>1800,'session_totp_past_steps'=>2,'auto_workspace'=>true,'auto_test'=>true,'auto_staging'=>true,'auto_memory'=>true,'auto_goal'=>true,'auto_memory_archive'=>true,'auto_yellow_from_session'=>true,'internal_self_update'=>true,'red_requires_totp'=>false,'production_requires_totp'=>true,'kernel_requires_totp'=>false,'external_authority_requires_human'=>true,'archive_append_only'=>true];
...
function kicomAutonomySessionPeek(string $id,string $token): array {
...
function kicomTransportProfilesFile(): string {return kicomAuthDir().'/transport-profiles.json';}
...
function kicomTransportProfilesRead(): array {$r=function_exists('kicomSqliteKvRead')?kicomSqliteKvRead('transport','profiles',kicomTransportProfilesFile(),['schema'=>1,'profiles'=>[]]):kicomAuthJsonRead(kicomTransportProfilesFile());if(!is_array($r)||!is_array($r['profiles']??null))$r=['schema'=>1,'profiles'=>[]];return $r;}
function kicomTransportProfileView(string $id): array {$id=kicomTransportProfileId($id);if($id===null)return ['ok'=>false,'code'=>'TX_PROFILE_INVALID'];$a=kicomTransportProfilesRead();$p=is_array($a['profiles'][$id]??null)?$a['profiles'][$id]:[];$mx=8192;$safe=max(384,min($mx,(int)($p['safe_fragment_bytes']??768)));$up=max(0,min($mx,(int)($p['failed_upper_bound']??0)));$probe=max($safe+128,(int)floor($safe*1.25));if($up>$safe)$probe=min($probe,max($safe,$up-64));$probe=min($mx,$probe);return ['ok'=>true,'profile'=>$id,'safe_fragment_bytes'=>$safe,'probe_fragment_bytes'=>$probe,'server_max_fragment_bytes'=>$mx,'failed_upper_bound'=>$up,'successes'=>(int)($p['successes']??0),'failures'=>(int)($p['failures']??0)];}
function kicomTransportProfileSuccess(string $id,int $bytes): array {$v=kicomTransportProfileView($id);if(empty($v['ok']))return $v;$id=(string)$v['profile'];$a=kicomTransportProfilesRead();$p=is_array($a['profiles'][$id]??null)?$a['profiles'][$id]:[];$bytes=max(0,min((int)$v['server_max_fragment_bytes'],$bytes));$safe=max((int)$v['safe_fragment_bytes'],$bytes);$up=(int)$v['failed_upper_bound'];if($up>0&&$bytes>=$up)$up=0;$p['safe_fragment_bytes']=$safe;$p['failed_upper_bound']=$up;$p['successes']=(int)($p['successes']??0)+1;$p['last_success_at']=gmdate('c');$p['last_success_bytes']=$bytes;$a['profiles'][$id]=$p;$a['updated_at']=gmdate('c');if(!(function_exists('kicomSqliteKvWrite')?kicomSqliteKvWrite('transport','profiles',$a,kicomTransportProfilesFile()):kicomAuthJsonWrite(kicomTransportProfilesFile(),$a)))return ['ok'=>false,'code'=>'TX_PROFILE_WRITE_FAILED'];return kicomTransportProfileView($id);}
function kicomTransportProfileBackoff(string $id,int $failed,int $good=0): array {$v=kicomTransportProfileView($id);if(empty($v['ok']))return $v;$id=(string)$v['profile'];$mx=(int)$v['server_max_fragment_bytes'];if($failed<385)return ['ok'=>false,'code'=>'TX_BACKOFF_INVALID'];$failed=min($mx,$failed);$good=max(0,min($mx,$good));$a=kicomTransportProfilesRead();$p=is_array($a['profiles'][$id]??null)?$a['profiles'][$id]:[];$safe=(int)$v['safe_fragment_bytes'];$cand=$good>0?$good:(int)floor($failed*.70);$cand=max(384,min($safe,$cand));$up=(int)$v['failed_upper_bound'];$up=$up>0?min($up,$failed):$failed;$p['safe_fragment_bytes']=$cand;$p['failed_upper_bound']=$up;$p['failures']=(int)($p['failures']??0)+1;$p['last_failure_at']=gmdate('c');$p['last_failure_bytes']=$failed;$a['profiles'][$id]=$p;$a['updated_at']=gmdate('c');if(!(function_exists('kicomSqliteKvWrite')?kicomSqliteKvWrite('transport','profiles',$a,kicomTransportProfilesFile()):kicomAuthJsonWrite(kicomTransportProfilesFile(),$a)))return ['ok'=>false,'code'=>'TX_PROFILE_WRITE_FAILED'];return kicomTransportProfileView($id);}
...
function kicomAutonomyTxBegin(string $sid,string $stok,string $profile='chat_get'): array {$s=kicomAutonomySessionPeek($sid,$stok);if(empty($s['ok']))return $s;kicomAuthTxCleanup();$pv=kicomTransportProfileView($profile);if(empty($pv['ok']))return $pv;$n=0;foreach(glob(kicomAuthDir().'/tx-*.json')?:[] as $f){$z=kicomAuthJsonRead($f);if(is_array($z)&&hash_equals((string)($z['session_id']??''),$sid))$n++;}if($n>=4)return ['ok'=>false,'code'=>'TX_LIMIT'];$id=substr(kicomAuthRandomHex(10),0,20);$tok=kicomAuthRandomHex(32);$now=time();$exp=min((int)$s['expires_at'],(int)$s['idle_expires_at'],$now+600);$row=['schema'=>2,'id'=>$id,'session_id'=>$sid,'token_hash'=>hash('sha256',$tok),'created_at'=>gmdate('c'),'expires_at'=>$exp,'next_seq'=>0,'bytes'=>0,'payload'=>'','max_bytes'=>65536,'profile'=>(string)$pv['profile'],'max_fragment_bytes'=>(int)$pv['server_max_fragment_bytes'],'safe_fragment_bytes'=>(int)$pv['safe_fragment_bytes'],'probe_fragment_bytes'=>(int)$pv['probe_fragment_bytes'],'last_seq'=>-1,'last_chunk_sha256'=>''];if(!kicomAuthJsonWrite(kicomAuthTxFile($id),$row))return ['ok'=>false,'code'=>'TX_WRITE_FAILED'];return ['ok'=>true,'tx_id'=>$id,'tx_token'=>$tok,'expires_in'=>max(0,$exp-$now),'max_bytes'=>65536,'profile'=>(string)$pv['profile'],'safe_fragment_bytes'=>(int)$pv['safe_fragment_bytes'],'probe_fragment_bytes'=>(int)$pv['probe_fragment_bytes'],'server_max_fragment_bytes'=>(int)$pv['server_max_fragment_bytes']];}
function kicomAutonomyTxAppend(string $id,string $tok,int $seq,string $data): array {$z=kicomAutonomyTxGet($id,$tok);if(empty($z['ok']))return $z;$r=$z['row'];$c=kicomDecodeBase64Url($data,true);if($c===null)return ['ok'=>false,'code'=>'TX_CHUNK_ENCODING'];$hs=hash('sha256',$c);$next=(int)($r['next_seq']??0);if($seq!==$next){if($seq===$next-1&&(int)($r['last_seq']??-2)===$seq&&hash_equals((string)($r['last_chunk_sha256']??''),$hs)){$pv=kicomTransportProfileView((string)($r['profile']??'chat_get'));return ['ok'=>true,'code'=>'OK','tx_id'=>$id,'next_seq'=>$next,'bytes'=>(int)($r['bytes']??0),'sha256'=>hash('sha256',(string)($r['payload']??'')),'replayed'=>true]+$pv;}return ['ok'=>false,'code'=>'TX_SEQ','next_seq'=>$next];}$mx=(int)($r['max_fragment_bytes']??8192);if(strlen($c)>$mx)return ['ok'=>false,'code'=>'TX_FRAGMENT_TOO_LARGE','max_fragment_bytes'=>$mx];$p=(string)($r['payload']??'').$c;if(strlen($p)>(int)($r['max_bytes']??65536))return ['ok'=>false,'code'=>'TX_TOO_LARGE'];$r['payload']=$p;$r['bytes']=strlen($p);$r['next_seq']=$seq+1;$r['last_seq']=$seq;$r['last_chunk_sha256']=$hs;$r['updated_at']=gmdate('c');if(!kicomAuthJsonWrite($z['file'],$r))return ['ok'=>false,'code'=>'TX_WRITE_FAILED'];$pv=kicomTransportProfileSuccess((string)($r['profile']??'chat_get'),strlen($c));if(empty($pv['ok']))return $pv;$r['safe_fragment_bytes']=(int)$pv['safe_fragment_bytes'];$r['probe_fragment_bytes']=(int)$pv['probe_fragment_bytes'];kicomAuthJsonWrite($z['file'],$r);return ['ok'=>true,'code'=>'OK','tx_id'=>$id,'next_seq'=>$seq+1,'bytes'=>strlen($p),'sha256'=>hash('sha256',$p),'replayed'=>false]+$pv;}
function kicomAutonomyTxBackoff(string $id,string $tok,int $failed,int $good=0): array {$z=kicomAutonomyTxGet($id,$tok);if(empty($z['ok']))return $z;$r=$z['row'];$pv=kicomTransportProfileBackoff((string)($r['profile']??'chat_get'),$failed,$good);if(empty($pv['ok']))return $pv;$r['safe_fragment_bytes']=(int)$pv['safe_fragment_bytes'];$r['probe_fragment_bytes']=(int)$pv['probe_fragment_bytes'];$r['updated_at']=gmdate('c');if(!kicomAuthJsonWrite($z['file'],$r))return ['ok'=>false,'code'=>'TX_WRITE_FAILED'];return ['ok'=>true,'code'=>'OK','tx_id'=>$id]+$pv;}

/* KiCom Semantic Codebook Transport 1 */
function kicomCodebookVersion(): string { return 'KCL-CODEBOOK/1'; }
function kicomCodebookResources(): array {
    return ['P'=>'PROJECT_STATE','A'=>'ARCHITECTURE','R'=>'PROTOCOL','D'=>'DECISIONS','C'=>'CHANGELOG','N'=>'NEXT'];
}
function kicomCodebookTerminators(): array {
    return [
        'PROJECT_STATE'=>'END_PROJECT kicom',
        'ARCHITECTURE'=>'END_ARCHITECTURE kicom',
        'PROTOCOL'=>'END_PROTOCOL KCL/1',
        'DECISIONS'=>'END_DECISIONS kicom',
        'CHANGELOG'=>'END_CHANGELOG kicom',
        'NEXT'=>'END_NEXT kicom',
    ];
}
function kicomCodebookSpec(): array {
    return [
        'version'=>kicomCodebookVersion(),
        'resources'=>kicomCodebookResources(),
        'opcodes'=>[
            'p'=>'exact canonical-memory patch',
            'a'=>'append KCL before canonical terminator',
            'e'=>'append CHANGELOG event using current UTC date',
            'd'=>'append DECISION/RATIONALE pair',
            'n'=>'replace NEXT priority goal line',
        ],
        'text_grammar'=>'segments separated by ~; pR.find_b64.replace_b64 | aR.payload_b64 | e.change_b64 | dNNN.title_b64.rationale_b64 | nN.goal_b64',
        'codecs'=>[
            'r'=>'raw URL-safe text grammar',
            'z'=>'base64url(gzip(binary TLV)); binary header CB1, one-byte op/resource codes and big-endian u16 field lengths',
        ],
        'max_operations'=>32,
        'max_program_bytes'=>65536,
        'context_ttl'=>600,
        'security'=>'context captures exact full SHA-256 for canonical resources; execution requires live autonomy session token and unchanged snapshot; preflight validates all targets before write and compensating rollback is attempted on write failure',
    ];
}
function kicomCodebookSha256(): string {
    $j=json_encode(kicomCodebookSpec(),JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE);
    return hash('sha256',$j===false?'':$j);
}
function kicomCodebookDir(): string { return kicomAuthDir().'/codebook'; }
function kicomCodebookStatsFile(): string { return kicomCodebookDir().'/stats.json'; }
function kicomCodebookEnsure(): bool {
    $d=kicomCodebookDir();
    return (is_dir($d)||@mkdir($d,0700,true)||is_dir($d));
}
function kicomCodebookContextFile(string $id): string { return kicomCodebookDir().'/ctx-'.$id.'.json'; }
function kicomCodebookCleanup(): void {
    if(!kicomCodebookEnsure())return;
    $now=time();
    foreach(glob(kicomCodebookDir().'/ctx-*.json')?:[] as $f){
        $r=kicomAuthJsonRead($f);
        if(!is_array($r)||(int)($r['expires_at']??0)<$now)@unlink($f);
    }
}
function kicomCodebookBegin(string $sid,string $stok): array {
    $s=kicomAutonomySessionPeek(strtolower(trim($sid)),strtolower(trim($stok)));
    if(empty($s['ok']))return $s;
    if(!kicomCodebookEnsure())return ['ok'=>false,'code'=>'CODEBOOK_STORAGE_UNAVAILABLE'];
    kicomCodebookCleanup();
    $shas=[];
    foreach(kicomCodebookResources() as $name){
        $r=kicomReadMemoryResource($name);
        if($r===null)return ['ok'=>false,'code'=>'CODEBOOK_RESOURCE_UNAVAILABLE','resource'=>$name];
        $shas[$name]=(string)$r['sha256'];
    }
    $id=substr(kicomAuthRandomHex(10),0,20);
    $now=time();$exp=min((int)$s['expires_at'],(int)$s['idle_expires_at'],$now+600);
    $row=[
        'schema'=>1,'id'=>$id,'session_id'=>(string)$s['session_id'],
        'codebook'=>kicomCodebookVersion(),'codebook_sha256'=>kicomCodebookSha256(),
        'created_at'=>gmdate('c'),'expires_at'=>$exp,'resource_sha256'=>$shas,
    ];
    if(!kicomAuthJsonWrite(kicomCodebookContextFile($id),$row))return ['ok'=>false,'code'=>'CODEBOOK_CONTEXT_WRITE_FAILED'];
    return [
        'ok'=>true,'code'=>'OK','context_id'=>$id,'expires_in'=>max(0,$exp-$now),
        'codebook'=>kicomCodebookVersion(),'codebook_sha256'=>kicomCodebookSha256(),
        'gzip_available'=>function_exists('gzdecode'),
        'resources'=>kicomCodebookResources(),
    ];
}
function kicomCodebookContext(string $sid,string $id): array {
    kicomCodebookCleanup();
    $id=strtolower(trim($id));$sid=strtolower(trim($sid));
    if(!preg_match('/^[a-f0-9]{20}$/',$id))return ['ok'=>false,'code'=>'CODEBOOK_CONTEXT_INVALID'];
    $f=kicomCodebookContextFile($id);$r=kicomAuthJsonRead($f);
    if(!is_array($r)||!hash_equals((string)($r['session_id']??''),$sid))return ['ok'=>false,'code'=>'CODEBOOK_CONTEXT_NOT_FOUND'];
    if((int)($r['expires_at']??0)<time()){ @unlink($f);return ['ok'=>false,'code'=>'CODEBOOK_CONTEXT_EXPIRED'];}
    if(!hash_equals((string)($r['codebook_sha256']??''),kicomCodebookSha256()))return ['ok'=>false,'code'=>'CODEBOOK_VERSION_MISMATCH'];
    return ['ok'=>true,'file'=>$f,'row'=>$r];
}
function kicomCodebookDecodeEnvelope(string $codec,string $data): array {
    $codec=strtolower(trim($codec));
    if(!in_array($codec,['r','z'],true))return ['ok'=>false,'code'=>'CODEBOOK_CODEC_UNSUPPORTED'];
    if($data===''||strlen($data)>32768)return ['ok'=>false,'code'=>'CODEBOOK_DATA_SIZE_INVALID'];
    if($codec==='r'){
        if(strlen($data)>65536)return ['ok'=>false,'code'=>'CODEBOOK_PROGRAM_TOO_LARGE'];
        return ['ok'=>true,'codec'=>'r','payload'=>$data,'format'=>'text','wire_bytes'=>strlen($data),'program_bytes'=>strlen($data)];
    }
    $wire=kicomDecodeBase64Url($data,false);
    if($wire===null)return ['ok'=>false,'code'=>'CODEBOOK_DATA_ENCODING'];
    if(strlen($wire)>24576)return ['ok'=>false,'code'=>'CODEBOOK_WIRE_TOO_LARGE'];
    if(!function_exists('gzdecode'))return ['ok'=>false,'code'=>'CODEBOOK_GZIP_UNAVAILABLE'];
    $program=@gzdecode($wire,65536);
    if($program===false)return ['ok'=>false,'code'=>'CODEBOOK_GZIP_INVALID'];
    if(strlen($program)>65536)return ['ok'=>false,'code'=>'CODEBOOK_PROGRAM_TOO_LARGE'];
    if(strlen($wire)>0&&strlen($program)>max(4096,strlen($wire)*128))return ['ok'=>false,'code'=>'CODEBOOK_EXPANSION_RATIO'];
    return ['ok'=>true,'codec'=>'z','payload'=>$program,'format'=>'binary','wire_bytes'=>strlen($wire),'program_bytes'=>strlen($program)];
}
function kicomCodebookB64(string $v,int $max,string $field): array {
    if(!preg_match('/^[A-Za-z0-9_-]*$/',$v))return ['ok'=>false,'code'=>'CODEBOOK_FIELD_ENCODING','field'=>$field];
    $d=kicomDecodeBase64Url($v,true);
    if($d===null||strlen($d)>$max)return ['ok'=>false,'code'=>'CODEBOOK_FIELD_SIZE','field'=>$field];
    return ['ok'=>true,'value'=>$d];
}
function kicomCodebookParseText(string $program): array {
    $segments=explode('~',$program);
    if(count($segments)<1||count($segments)>32)return ['ok'=>false,'code'=>'CODEBOOK_OP_COUNT'];
    $ops=[];$rc=kicomCodebookResources();
    foreach($segments as $i=>$seg){
        if($seg==='')return ['ok'=>false,'code'=>'CODEBOOK_EMPTY_OP','index'=>$i];
        if(preg_match('/^p([PARDNC])\.([A-Za-z0-9_-]*)\.([A-Za-z0-9_-]*)$/',$seg,$m)){
            $f=kicomCodebookB64($m[2],2048,'find');if(empty($f['ok']))return $f+['index'=>$i];
            $r=kicomCodebookB64($m[3],4096,'replace');if(empty($r['ok']))return $r+['index'=>$i];
            if((string)$f['value']==='')return ['ok'=>false,'code'=>'CODEBOOK_FIND_EMPTY','index'=>$i];
            $ops[]=['op'=>'p','resource'=>$rc[$m[1]],'find'=>(string)$f['value'],'replace'=>(string)$r['value']];
            continue;
        }
        if(preg_match('/^a([PARDNC])\.([A-Za-z0-9_-]+)$/',$seg,$m)){
            $p=kicomCodebookB64($m[2],3500,'payload');if(empty($p['ok']))return $p+['index'=>$i];
            $ops[]=['op'=>'a','resource'=>$rc[$m[1]],'payload'=>(string)$p['value']];continue;
        }
        if(preg_match('/^e\.([A-Za-z0-9_-]+)$/',$seg,$m)){
            $p=kicomCodebookB64($m[1],1800,'change');if(empty($p['ok']))return $p+['index'=>$i];
            $ops[]=['op'=>'e','resource'=>'CHANGELOG','change'=>(string)$p['value']];continue;
        }
        if(preg_match('/^d([0-9]{1,5})\.([A-Za-z0-9_-]+)\.([A-Za-z0-9_-]+)$/',$seg,$m)){
            $t=kicomCodebookB64($m[2],700,'title');if(empty($t['ok']))return $t+['index'=>$i];
            $r=kicomCodebookB64($m[3],1800,'rationale');if(empty($r['ok']))return $r+['index'=>$i];
            $ops[]=['op'=>'d','resource'=>'DECISIONS','id'=>'D'.str_pad($m[1],3,'0',STR_PAD_LEFT),'title'=>(string)$t['value'],'rationale'=>(string)$r['value']];continue;
        }
        if(preg_match('/^n([1-9])\.([A-Za-z0-9_-]+)$/',$seg,$m)){
            $g=kicomCodebookB64($m[2],2200,'goal');if(empty($g['ok']))return $g+['index'=>$i];
            $ops[]=['op'=>'n','resource'=>'NEXT','priority'=>(int)$m[1],'goal'=>(string)$g['value']];continue;
        }
        return ['ok'=>false,'code'=>'CODEBOOK_OPCODE_INVALID','index'=>$i];
    }
    return ['ok'=>true,'ops'=>$ops];
}
function kicomCodebookReadU16(string $b,int &$p): ?int {
    if($p+2>strlen($b))return null;$u=unpack('nvalue',substr($b,$p,2));$p+=2;return is_array($u)?(int)$u['value']:null;
}
function kicomCodebookReadField(string $b,int &$p,int $max,string $field): array {
    $n=kicomCodebookReadU16($b,$p);if($n===null||$n<0||$n>$max||$p+$n>strlen($b))return ['ok'=>false,'code'=>'CODEBOOK_BINARY_FIELD','field'=>$field];
    $v=substr($b,$p,$n);$p+=$n;return ['ok'=>true,'value'=>$v];
}
function kicomCodebookParseBinary(string $b): array {
    if(strlen($b)<5||substr($b,0,3)!=='CB1')return ['ok'=>false,'code'=>'CODEBOOK_BINARY_HEADER'];
    $p=3;$count=ord($b[$p++]);if($count<1||$count>32)return ['ok'=>false,'code'=>'CODEBOOK_OP_COUNT'];
    $ops=[];$rc=kicomCodebookResources();
    for($i=0;$i<$count;$i++){
        if($p>=strlen($b))return ['ok'=>false,'code'=>'CODEBOOK_BINARY_TRUNCATED','index'=>$i];
        $op=$b[$p++];
        if($op==='p'){
            if($p>=strlen($b)||!isset($rc[$b[$p]]))return ['ok'=>false,'code'=>'CODEBOOK_RESOURCE_CODE','index'=>$i];$resource=$rc[$b[$p++]];
            $f=kicomCodebookReadField($b,$p,2048,'find');if(empty($f['ok']))return $f+['index'=>$i];
            $r=kicomCodebookReadField($b,$p,4096,'replace');if(empty($r['ok']))return $r+['index'=>$i];
            if((string)$f['value']==='')return ['ok'=>false,'code'=>'CODEBOOK_FIND_EMPTY','index'=>$i];
            $ops[]=['op'=>'p','resource'=>$resource,'find'=>(string)$f['value'],'replace'=>(string)$r['value']];continue;
        }
        if($op==='a'){
            if($p>=strlen($b)||!isset($rc[$b[$p]]))return ['ok'=>false,'code'=>'CODEBOOK_RESOURCE_CODE','index'=>$i];$resource=$rc[$b[$p++]];
            $v=kicomCodebookReadField($b,$p,3500,'payload');if(empty($v['ok']))return $v+['index'=>$i];
            $ops[]=['op'=>'a','resource'=>$resource,'payload'=>(string)$v['value']];continue;
        }
        if($op==='e'){
            $v=kicomCodebookReadField($b,$p,1800,'change');if(empty($v['ok']))return $v+['index'=>$i];
            $ops[]=['op'=>'e','resource'=>'CHANGELOG','change'=>(string)$v['value']];continue;
        }
        if($op==='d'){
            $id=kicomCodebookReadU16($b,$p);if($id===null||$id<1)return ['ok'=>false,'code'=>'CODEBOOK_DECISION_ID','index'=>$i];
            $t=kicomCodebookReadField($b,$p,700,'title');if(empty($t['ok']))return $t+['index'=>$i];
            $r=kicomCodebookReadField($b,$p,1800,'rationale');if(empty($r['ok']))return $r+['index'=>$i];
            $ops[]=['op'=>'d','resource'=>'DECISIONS','id'=>'D'.str_pad((string)$id,3,'0',STR_PAD_LEFT),'title'=>(string)$t['value'],'rationale'=>(string)$r['value']];continue;
        }
        if($op==='n'){
            if($p>=strlen($b))return ['ok'=>false,'code'=>'CODEBOOK_BINARY_TRUNCATED','index'=>$i];$priority=ord($b[$p++]);
            if($priority<1||$priority>9)return ['ok'=>false,'code'=>'CODEBOOK_PRIORITY','index'=>$i];
            $g=kicomCodebookReadField($b,$p,2200,'goal');if(empty($g['ok']))return $g+['index'=>$i];
            $ops[]=['op'=>'n','resource'=>'NEXT','priority'=>$priority,'goal'=>(string)$g['value']];continue;
        }
        return ['ok'=>false,'code'=>'CODEBOOK_OPCODE_INVALID','index'=>$i];
    }
    if($p!==strlen($b))return ['ok'=>false,'code'=>'CODEBOOK_BINARY_TRAILING_BYTES'];
    return ['ok'=>true,'ops'=>$ops];
}
function kicomCodebookParseEnvelope(array $env): array {
    return ($env['format']??'')==='binary'?kicomCodebookParseBinary((string)$env['payload']):kicomCodebookParseText((string)$env['payload']);
}
function kicomCodebookKclEscape(string $v): string {
    return str_replace(["\\","\r","\n","\t",'"'],["\\\\","\\r","\\n","\\t",'\"'],$v);
}
function kicomCodebookApplyOne(string $resource,string $raw,array $op): array {
    $terms=kicomCodebookTerminators();$term=$terms[$resource]??'';
    $find='';$replace='';
    switch((string)$op['op']){
        case 'p': $find=(string)$op['find'];$replace=(string)$op['replace'];break;
        case 'a':
            if($term==='')return ['ok'=>false,'code'=>'CODEBOOK_TERMINATOR_MISSING'];
            $find=$term;$payload=rtrim((string)$op['payload'],"\r\n");
            if($payload==='')return ['ok'=>false,'code'=>'CODEBOOK_PAYLOAD_EMPTY'];
            $replace=$payload."\n".$term;break;
        case 'e':
            $find=$term;$change=kicomCodebookKclEscape((string)$op['change']);
            if($change==='')return ['ok'=>false,'code'=>'CODEBOOK_CHANGE_EMPTY'];
            $replace='EVENT "'.gmdate('Y-m-d').'" change="'.$change.'"'."\n".$term;break;
        case 'd':
            $find=$term;$id=(string)$op['id'];
            if(preg_match('/^DECISION\s+'.preg_quote($id,'/').'\b/m',$raw))return ['ok'=>false,'code'=>'CODEBOOK_DECISION_EXISTS'];
            $title=kicomCodebookKclEscape((string)$op['title']);$why=kicomCodebookKclEscape((string)$op['rationale']);
            if($title===''||$why==='')return ['ok'=>false,'code'=>'CODEBOOK_DECISION_EMPTY'];
            $replace='DECISION '.$id.' status=accepted title="'.$title.'"'."\n".'RATIONALE '.$id.' "'.$why.'"'."\n".$term;break;
        case 'n':
            $p=(int)$op['priority'];$goal=kicomCodebookKclEscape((string)$op['goal']);
            if($goal==='')return ['ok'=>false,'code'=>'CODEBOOK_GOAL_EMPTY'];
            if(!preg_match('/^PRIORITY\s+'.preg_quote((string)$p,'/').'\s+goal=".*"$/m',$raw,$mm))return ['ok'=>false,'code'=>'CODEBOOK_PRIORITY_NOT_FOUND'];
            $find=(string)$mm[0];$replace='PRIORITY '.$p.' goal="'.$goal.'"';break;
        default:return ['ok'=>false,'code'=>'CODEBOOK_OPCODE_UNSUPPORTED'];
    }
    if($find===''||strlen($find)>2048||strlen($replace)>4096)return ['ok'=>false,'code'=>'CODEBOOK_EXPANDED_PATCH_SIZE'];
    $count=substr_count($raw,$find);if($count!==1)return ['ok'=>false,'code'=>'CODEBOOK_PATCH_MATCH_COUNT','matches'=>$count];
    $next=str_replace($find,$replace,$raw,$n);
    if($n!==1||strlen($next)>6144)return ['ok'=>false,'code'=>'CODEBOOK_CANDIDATE_SIZE'];
    $v=kicomValidateMemoryContent($resource,$next);
    if(($v['status']??'')==='error')return ['ok'=>false,'code'=>'CODEBOOK_VALIDATION_FAILED','validation'=>$v['message']];
    return ['ok'=>true,'raw'=>$next,'find'=>$find,'replace'=>$replace,'sha256'=>hash('sha256',$next)];
}
function kicomCodebookPlan(array $ctx,array $ops): array {
    $snap=$ctx['resource_sha256']??[];if(!is_array($snap))return ['ok'=>false,'code'=>'CODEBOOK_CONTEXT_SCHEMA'];
    $shadow=[];$initial=[];$final=[];$expanded=0;$targets=[];
    foreach($ops as $i=>$op){
        $resource=(string)($op['resource']??'');
        if(!isset($snap[$resource]))return ['ok'=>false,'code'=>'CODEBOOK_RESOURCE_NOT_SNAPSHOTTED','index'=>$i];
        if(!isset($shadow[$resource])){
            $cur=kicomReadMemoryResource($resource);
            if($cur===null)return ['ok'=>false,'code'=>'CODEBOOK_RESOURCE_UNAVAILABLE','resource'=>$resource];
            if(!hash_equals((string)$snap[$resource],(string)$cur['sha256']))return ['ok'=>false,'code'=>'CODEBOOK_CONTEXT_STALE','resource'=>$resource,'current_sha256'=>$cur['sha256']];
            $shadow[$resource]=(string)$cur['raw'];$initial[$resource]=(string)$cur['raw'];$targets[]=$resource;
        }
        $a=kicomCodebookApplyOne($resource,$shadow[$resource],$op);
        if(empty($a['ok']))return $a+['index'=>$i,'resource'=>$resource];
        $shadow[$resource]=(string)$a['raw'];
        $expanded+=strlen(json_encode(['kind'=>'memory_patch','resource'=>$resource,'base_sha256'=>(string)$snap[$resource],'find'=>(string)$a['find'],'replace'=>(string)$a['replace']],JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE)?:'');
    }
    foreach($shadow as $r=>$raw)$final[$r]=['raw'=>$raw,'sha256'=>hash('sha256',$raw)];
    return ['ok'=>true,'initial'=>$initial,'final'=>$final,'initial_sha256'=>$snap,'targets'=>$targets,'expanded_json_bytes'=>$expanded,'operations'=>count($ops)];
}
function kicomCodebookStatsRecord(array $row): void {
    if(!kicomCodebookEnsure())return;
    $f=kicomCodebookStatsFile();$s=kicomAuthJsonRead($f);
    if(!is_array($s))$s=['schema'=>1,'executions'=>0,'wire_bytes'=>0,'program_bytes'=>0,'expanded_json_bytes'=>0,'operations'=>0,'by_codec'=>[]];
    $s['executions']=(int)($s['executions']??0)+1;
    foreach(['wire_bytes','program_bytes','expanded_json_bytes','operations'] as $k)$s[$k]=(int)($s[$k]??0)+(int)($row[$k]??0);
    $c=(string)($row['codec']??'unknown');if(!isset($s['by_codec'][$c]))$s['by_codec'][$c]=0;$s['by_codec'][$c]++;
    $s['last_at']=gmdate('c');$s['last']=$row;kicomAuthJsonWrite($f,$s);
}
function kicomCodebookStats(): array {
    if(!kicomCodebookEnsure())return ['ok'=>false,'code'=>'CODEBOOK_STORAGE_UNAVAILABLE'];
    $s=kicomAuthJsonRead(kicomCodebookStatsFile());if(!is_array($s))$s=['schema'=>1,'executions'=>0,'wire_bytes'=>0,'program_bytes'=>0,'expanded_json_bytes'=>0,'operations'=>0,'by_codec'=>[]];
    return ['ok'=>true,'stats'=>$s];
}
function kicomCodebookRun(string $sid,string $stok,string $contextId,string $codec,string $data,bool $execute): array {
    $cx=kicomCodebookContext($sid,$contextId);if(empty($cx['ok']))return $cx;
    $env=kicomCodebookDecodeEnvelope($codec,$data);if(empty($env['ok']))return $env;
    $parsed=kicomCodebookParseEnvelope($env);if(empty($parsed['ok']))return $parsed;
    $plan=kicomCodebookPlan($cx['row'],$parsed['ops']);if(empty($plan['ok']))return $plan;
    if(!$execute){
        $s=kicomAutonomySessionPeek(strtolower(trim($sid)),strtolower(trim($stok)));if(empty($s['ok']))return $s;
        return ['ok'=>true,'code'=>'OK','dry_run'=>true,'operations'=>$plan['operations'],'targets'=>$plan['targets'],'wire_bytes'=>$env['wire_bytes'],'program_bytes'=>$env['program_bytes'],'expanded_json_bytes'=>$plan['expanded_json_bytes'],'codebook_sha256'=>kicomCodebookSha256()];
    }
    $ss=kicomAutonomySessionConsume(strtolower(trim($sid)),strtolower(trim($stok)));if(empty($ss['ok']))return $ss;
    $written=[];$results=[];$ok=true;$fail=null;
    foreach($plan['targets'] as $resource){
        $before=(string)$plan['initial'][$resource];$base=(string)$plan['initial_sha256'][$resource];$after=(string)$plan['final'][$resource]['raw'];
        if(hash_equals(hash('sha256',$before),hash('sha256',$after))){$results[]=['resource'=>$resource,'ok'=>true,'code'=>'NO_CHANGE','sha256'=>$base];continue;}
        $r=kicomAtomicMemoryWrite($resource,$after,'codebook_batch',$base);
        $results[]=['resource'=>$resource]+$r;
        if(empty($r['ok'])){$ok=false;$fail=['resource'=>$resource,'result'=>$r];break;}
        $written[]=['resource'=>$resource,'before'=>$before,'after_sha256'=>(string)($r['sha256']??hash('sha256',$after))];
    }
    $rollback=[];$rollbackOk=true;
    if(!$ok){
        foreach(array_reverse($written) as $w){
            $rr=kicomAtomicMemoryWrite((string)$w['resource'],(string)$w['before'],'codebook_compensating_rollback',(string)$w['after_sha256']);
            $rollback[]=['resource'=>$w['resource']]+$rr;$rollbackOk=$rollbackOk&&!empty($rr['ok']);
        }
    }
    @unlink((string)$cx['file']);
    $metrics=['codec'=>$env['codec'],'wire_bytes'=>$env['wire_bytes'],'program_bytes'=>$env['program_bytes'],'expanded_json_bytes'=>$plan['expanded_json_bytes'],'operations'=>$plan['operations'],'success'=>$ok,'rollback_ok'=>$rollbackOk];
    kicomCodebookStatsRecord($metrics);
    if($ok)kicomLivingEvent('codebook_execute','info',$metrics);else kicomLivingEvent('codebook_execute_failed','warn',$metrics+['failure'=>$fail]);
    return [
        'ok'=>$ok,'code'=>$ok?'OK':($rollbackOk?'CODEBOOK_EXEC_FAILED_ROLLED_BACK':'CODEBOOK_EXEC_FAILED_ROLLBACK_INCOMPLETE'),
        'session_id'=>(string)$ss['session_id'],'next_token'=>(string)$ss['next_token'],
        'session_expires_at'=>(int)$ss['expires_at'],'session_idle_expires_at'=>(int)$ss['idle_expires_at'],
        'operations'=>$plan['operations'],'targets'=>$plan['targets'],'results'=>$results,'rollback'=>$rollback,
        'wire_bytes'=>$env['wire_bytes'],'program_bytes'=>$env['program_bytes'],'expanded_json_bytes'=>$plan['expanded_json_bytes'],
        'codebook_sha256'=>kicomCodebookSha256(),
    ];
}
...
function kicomAuthWsBatchItem(string $id): array {
    $id=kicomSafeId($id);if($id===null)return ['ok'=>false,'code'=>'BATCH_ID'];
    $file=kicomPendingDir().'/'.$id.'.json';$raw=@file_get_contents($file);$p=$raw===false?null:json_decode($raw,true);
    if(!is_array($p)||($p['kind']??'')!=='workspace_write'||strtoupper(trim((string)($p['base_sha256']??'')))!=='NEW')return ['ok'=>false,'code'=>'BATCH_PROPOSAL','proposal_id'=>$id];
    $path=kicomSafeRelativePath((string)($p['path']??''));$content=base64_decode((string)($p['content_b64']??''),true);
    if($path===null||$content===false||kicomCurrentHash($path)!=='NEW')return ['ok'=>false,'code'=>'BATCH_TARGET','proposal_id'=>$id];
    $sha=hash('sha256',$content);if(!hash_equals($sha,strtolower((string)($p['sha256']??''))))return ['ok'=>false,'code'=>'BATCH_CONTENT_SHA','proposal_id'=>$id];
    $v=kicomValidateContent($path,$content);if(($v['status']??'')==='error')return ['ok'=>false,'code'=>'BATCH_VALIDATE','proposal_id'=>$id];
    return ['ok'=>true,'id'=>$id,'file'=>$file,'proposal_sha256'=>hash('sha256',$raw),'path'=>$path,'content'=>$content,'content_sha256'=>$sha];
}
function kicomAuthPrepareWorkspaceProposalBatch(string $sid,string $csv): array {
    $ids=array_values(array_filter(array_map('trim',explode(',',$csv)),fn($v)=>$v!==''));
    if(count($ids)<1||count($ids)>16||count(array_unique($ids))!==count($ids))return ['ok'=>false,'code'=>'BATCH_IDS'];
    $entries=[];$paths=[];foreach($ids as $id){$s=kicomAuthWsBatchItem($id);if(empty($s['ok']))return $s;if(isset($paths[$s['path']]))return ['ok'=>false,'code'=>'BATCH_DUP_PATH'];$paths[$s['path']]=1;$entries[]=['id'=>$s['id'],'proposal_sha256'=>$s['proposal_sha256'],'path'=>$s['path'],'content_sha256'=>$s['content_sha256']];}
    $j=json_encode($entries,JSON_UNESCAPED_SLASHES);$binding=['action'=>'workspace_proposal_batch','proposal_count'=>count($entries),'proposal_ids'=>implode(',',$ids),'batch_sha256'=>hash('sha256',$j?:''),'proposals'=>$entries];
    return kicomAuthApprovalCreate($sid,'workspace_proposal_batch',$binding,[],'yellow');
}
function kicomAuthWsBatchEntry(array $e): array {
    $s=kicomAuthWsBatchItem((string)($e['id']??''));if(empty($s['ok']))return $s;
    if(!hash_equals((string)($e['proposal_sha256']??''),$s['proposal_sha256'])||!hash_equals((string)($e['content_sha256']??''),$s['content_sha256'])||!hash_equals((string)($e['path']??''),$s['path']))return ['ok'=>false,'code'=>'BATCH_CHANGED'];
    return $s;
}
function kicomAuthWsBatchResolve(array $binding): array {
    if(($binding['action']??'')!=='workspace_proposal_batch'||!is_array($binding['proposals']??null))return ['ok'=>false,'code'=>'BATCH_BINDING'];
    $entries=$binding['proposals'];if(count($entries)<1||count($entries)>16)return ['ok'=>false,'code'=>'BATCH_BINDING'];
    $items=[];$check=[];$paths=[];foreach($entries as $e){if(!is_array($e))return ['ok'=>false,'code'=>'BATCH_BINDING'];$s=kicomAuthWsBatchEntry($e);if(empty($s['ok']))return $s;if(isset($paths[$s['path']]))return ['ok'=>false,'code'=>'BATCH_DUP_PATH'];$paths[$s['path']]=1;$check[]=['id'=>$s['id'],'proposal_sha256'=>$s['proposal_sha256'],'path'=>$s['path'],'content_sha256'=>$s['content_sha256']];$items[]=$s;}
    $j=json_encode($check,JSON_UNESCAPED_SLASHES);if(!hash_equals((string)($binding['batch_sha256']??''),hash('sha256',$j?:'')))return ['ok'=>false,'code'=>'BATCH_CHANGED'];return ['ok'=>true,'items'=>$items];
}
function kicomAuthWsBatchRollback(array $done): bool {
    $ok=true;foreach(array_reverse($done) as $d){$t=kicomStageDir().'/'.$d['path'];$h=is_file($t)?hash_file('sha256',$t):false;if($h===false||!hash_equals($d['sha256'],$h)||!@unlink($t))$ok=false;}return $ok;
}
function kicomAuthWsBatchWrite(array $items): array {
    $done=[];foreach($items as $s){$r=kicomAtomicStageWrite($s['path'],$s['content'],'write','NEW');if(empty($r['ok']))return ['ok'=>false,'code'=>kicomAuthWsBatchRollback($done)?'BATCH_WRITE_FAILED':'BATCH_ROLLBACK_INCOMPLETE'];$done[]=['path'=>$s['path'],'sha256'=>$r['sha256']];}return ['ok'=>true,'done'=>$done];
}
function kicomAuthApplyWorkspaceProposalBatch(array $binding): array {
    $lock=@fopen(kicomAuthDir().'/workspace-proposal-batch.lock','c+');if($lock===false||!@flock($lock,LOCK_EX|LOCK_NB))return ['ok'=>false,'code'=>'BATCH_BUSY'];
    try{$q=kicomAuthWsBatchResolve($binding);if(empty($q['ok']))return $q;$items=$q['items'];$w=kicomAuthWsBatchWrite($items);if(empty($w['ok']))return $w;
        foreach($items as $s){$h=is_file($s['file'])?hash_file('sha256',$s['file']):false;if(is_string($h)&&hash_equals($s['proposal_sha256'],$h))@unlink($s['file']);}
        kicomLivingEvent('workspace_proposal_batch_applied','info',['count'=>count($w['done']),'batch_sha256'=>$binding['batch_sha256']??'']);return ['ok'=>true,'code'=>'WORKSPACE_PROPOSAL_BATCH_APPLIED','applied'=>count($w['done']),'batch_sha256'=>(string)($binding['batch_sha256']??'')];
    }finally{@flock($lock,LOCK_UN);@fclose($lock);}
}
/* KiCom 0.9.15 workspace proposal batch */
/* KiCom 0.9.15 post-release-prepare verified */
function kicomAuthApprovalExecute(string $id,string $code): array {
    ...
    elseif($action==='mail_recipient_allow')$result=kicomMailOutboundApplyRecipientApproval(is_array($row['binding']??null)?$row['binding']:[]);
    elseif($action==='workspace_proposal_batch')$result=kicomAuthApplyWorkspaceProposalBatch(is_array($row['binding']??null)?$row['binding']:[]);
...
function kicomAutonomyUploadFinish(string $sessionId,string $id): array {
    ...
    elseif($kind==='self_update'){
        $tmp=kicomTempDir().'/autonomy-'.substr(hash('sha256',$raw),0,20).'.zip';if(@file_put_contents($tmp,$raw,LOCK_EX)===false)$result=['ok'=>false,'code'=>'PACKAGE_TEMP_WRITE_FAILED'];else{try{$source=(string)($meta['source']??'autonomy:session');$result=kicomReceiveSelfUpdatePackage($tmp,(string)($meta['filename']??'KiCom-upload.zip'),$source,true);if(!empty($result['ok'])&&($result['code']??'')==='STAGED_DECISION_REQUIRED'){ $policy=kicomAutonomyPolicy();$pending=kicomSelfUpdatePending();if(is_array($pending)&&(!empty($policy['internal_self_update'])||(($result['risk_class']??($result['staged']['risk_class']??''))==='yellow'&&!empty($policy['auto_yellow_from_session']))))$result=kicomApplySelfUpdate($pending)+['code'=>'AUTO_INSTALLED_INTERNAL_SESSION'];}}finally{@unlink($tmp);}}
    }
