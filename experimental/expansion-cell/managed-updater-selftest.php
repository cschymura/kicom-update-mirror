...
try{
    ...
    // Thin active child -> Living v2 with perception/action memory.
    ...
    chk(is_file($cell2.'/lib/CellLiving.php')&&is_file($cell2.'/lib/CellPerceptionAction.php'),'PA runtime copied');
    ...
    $manifest=json_decode((string)file_get_contents($cell2.'/cell-manifest.json'),true);chk(is_array($manifest)&&($manifest['schema']??0)===3&&($manifest['perception_action_memory']??false)===true,'manifest v3');
    ...
    $pBefore=count(file($var2.'/living/perception/history.jsonl',FILE_IGNORE_NEW_LINES)?:[]);$aBefore=count(file($var2.'/living/action/history.jsonl',FILE_IGNORE_NEW_LINES)?:[]);
    ...
    chk(!empty($second['ok'])&&($second['code']??'')==='EXPANSION_LIVING_PA_UPGRADE_APPLIED','existing living PA upgrade');
    ...
    $rbSecond=$u->rollbackLivingRuntime($web2,$second);chk(!empty($rbSecond['ok']),'PA rollback');
