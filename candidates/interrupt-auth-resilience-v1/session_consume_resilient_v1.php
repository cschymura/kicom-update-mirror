...
/* Drop-in locked normal-session token consumer candidate.
 * Semantics match KiCom 0.9.12 rolling-token + 60s previous-token recovery,
 * while serializing consumption with the recovery path.
 */
function kicomAutonomySessionConsumeResilient(string $id,string $token): array {
    ...
    $file=kicomAutonomySessionFile($id);$lock=@fopen(kicomSessionRecoveryLockFile($id),'c+');
