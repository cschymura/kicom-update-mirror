...
/* Explicit session revocation rotates a delegation epoch; ordinary idle does not. */
$GLOBALS['builds'][$bid.'|admin.php']='admin';$a0=hash('sha256','admin');$a1=hash('sha256','ADMIN');$jv=kicomDelegatedBuildJobCreateV1($session,['build_id'=>$bid,'steps'=>[['path'=>'admin.php','base_sha256'=>$a0,'find'=>'admin','replace'=>'ADMIN','expected_sha256'=>$a1]]]);check(!empty($jv['ok']),'revocable job created');$vr=kicomDelegatedBuildJobReadV1($jv['job_id']);$epochBefore=(int)$vr['revocation_epoch'];$rot=kicomDelegatedBuildRotateRevocationEpochV1();check(!empty($rot['ok'])&&$rot['epoch']>$epochBefore,'revocation epoch rotates');$vx=kicomDelegatedBuildJobTickV1($jv['job_id']);check(($vx['code']??'')==='JOB_REVOKED','explicit revocation kills delegated job');check($GLOBALS['builds'][$bid.'|admin.php']==='admin','revoked job no write');$vs=kicomDelegatedBuildJobStatusV1($jv['job_id']);check($vs['state']==='REVOKED','revoked state retained');
