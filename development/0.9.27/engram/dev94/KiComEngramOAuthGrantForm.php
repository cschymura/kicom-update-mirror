    ...
    public static function parse(string $raw,array $trustedClient):array
    {
        ...
        if(($out['grant_type']??null)==='refresh_token'){
            ...
            if(array_key_exists('scope',$out)){
                if(!in_array($out['scope'],[self::READ_SCOPE,self::COMBINED_SCOPE],true))self::reject();
                $required[]='scope';
            }
            ...
            return ['kind'=>'refresh_token','arguments'=>['refresh_token'=>$out['refresh_token']]+(isset($out['scope'])?['requested_scope'=>$out['scope']]:[])];
        }
        self::reject();
    }
