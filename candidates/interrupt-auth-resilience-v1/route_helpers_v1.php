...
function kicomPendingInspectKclLinesV1(array $r,string $serverRequestId): array {
    ...
    if(!empty($r['pending'])){
        foreach(['from_version','to_version','zip_sha256','manifest_sha256','genome_id','genome_sha256','source','risk_class','created_at'] as $k)$lines[]='FACT '.$k.'='.kclString((string)($r[$k]??''));
        $lines[]='FACT kernel_update='.(!empty($r['kernel_update'])?'true':'false');$lines[]='FACT files_count='.(int)($r['files_count']??0);$lines[]='FACT install_files_count='.(int)($r['install_files_count']??0);
        foreach(($r['risk_reasons']??[]) as $i=>$x)$lines[]='RISK #'.($i+1).'. reason='.kclString((string)$x);
        foreach(($r['changed_paths']??[]) as $i=>$x)$lines[]='CHANGED #'.($i+1).'. path='.kclString((string)$x);
    }
