...
## Verified post-install source reconciliation (2026-09-21)

The full **original 0.9.31 package was recovered from the user's private file library and locally SHA-256+manifest checked**; its code already includes `api.php?q=ENGRAM_MCP` and all eight read-only MCP modules. Live KiCom is 0.9.31. Thus the earlier DEV-48 text that claimed the actual 0.9.31 source/route was unavailable is superseded. Do not re-create the route, install 0.9.31 again or downgrade to 0.9.27. Offline lint of 45 PHP files passed; two localhost-only unauthenticated probes returned 404 with empty body. No real private data were opened.

The remaining boundary is **independent host/VHost/UID/backup isolation evidence, exact operator-owned activation config, current passkey-owner/activation state, separately issued/revocable MCP bearer, and explicit ChatGPT connector setup**. Native admin `admin.php?engram_host_audit=1` is read-only but cannot itself establish all trust evidence. The browser connector was unavailable in the continuation session; do not pretend to have entered KiCom Admin. Read the appended correction in `DEV-48-HOST-INTEGRATION-CHECKPOINT.md` for the exact verified state. The 0.9.31 package stays in the private file library; do not publish package or private state to the public mirror.
