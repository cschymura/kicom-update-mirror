...
if(!empty($_SESSION['admin'])&&$_SERVER['REQUEST_METHOD']==='POST'&&isset($_POST['action'])){
    if(!validCsrf())$err='CSRF-Prüfung fehlgeschlagen.';
        ...
        } elseif($action==='mail_secret_save'){
            $mailSecret=(string)($_POST['mail_password']??'');$r=kicomMailSecretProvision($mailSecret);$mailSecret='';
            if(empty($r['ok']))$err='Mail-Passwort konnte nicht geschützt gespeichert werden: '.(string)($r['code']??'UNKNOWN');else $msg='KiCom-Mailpasswort geschützt gespeichert. Das Passwort wird nicht angezeigt oder protokolliert.';
        } elseif($action==='mail_probe'){
            $r=kicomMailProbe();if(empty($r['ok']))$err='Mail-Verbindungstest fehlgeschlagen: '.(string)($r['code']??'UNKNOWN');else $msg='IMAP und SMTP erfolgreich authentifiziert.';
        } elseif($action==='mail_loopback'){
            $r=kicomMailLoopback(20);if(empty($r['ok']))$err='Mail-Loopback fehlgeschlagen: '.(string)($r['code']??'UNKNOWN');else $msg='Mail-Loopback erfolgreich: KiCom hat seine eigene Nachricht im Posteingang wiedergefunden.';
        } elseif($action==='slack_secret_save'){
            $bot=(string)($_POST['slack_bot_token']??'');$sign=(string)($_POST['slack_signing_secret']??'');$r=kicomSlackSecretProvision($bot,$sign);$bot='';$sign='';
            if(empty($r['ok']))$err='Slack-Secrets konnten nicht geschützt gespeichert werden: '.(string)($r['code']??'UNKNOWN');else $msg='Slack Bot Token und Signing Secret geschützt gespeichert. Die Werte werden nicht angezeigt oder protokolliert.';
        } elseif($action==='slack_probe'){
            $r=kicomSlackProbe();if(empty($r['ok']))$err='Slack-Verbindungstest fehlgeschlagen: '.(string)($r['code']??'UNKNOWN');else $msg='Slack-Bot erfolgreich authentifiziert und dem Workspace KIcom zugeordnet.';
...
<section>
  <h2>KiCom E-Mail</h2>
  <?php $ms=kicomMailStatus();?>
  <div class="grid">
    <div class="card"><strong><?=h((string)$ms['identity'])?></strong><span class="big-state"><?=!empty($ms['configured'])?'BEREIT ZUM TEST':'PASSWORT FEHLT'?></span><p class="muted">IMAPS <?=h((string)$ms['imap_host'])?>:<?=h((string)$ms['imap_port'])?> · SMTPS <?=h((string)$ms['smtp_host'])?>:<?=h((string)$ms['smtp_port'])?> · TLS-Zertifikatsprüfung aktiv.</p></div>
    <div class="card"><strong>Geschütztes Mailpasswort</strong><p class="muted">Wird ausschließlich unter <code>var/secrets</code> bzw. über die Server-Umgebung gelesen. Es erscheint nicht in KiCom Memory, GitHub oder Logs.</p><form method="post" autocomplete="off"><input type="hidden" name="csrf" value="<?=h(csrf())?>"><input type="hidden" name="action" value="mail_secret_save"><input type="password" name="mail_password" required autocomplete="new-password" placeholder="Mailpasswort"> <button class="approve">Geschützt speichern</button></form></div>
  </div>
  <?php if(!empty($ms['configured'])):?><div class="action-row" style="margin-top:14px"><form method="post"><input type="hidden" name="csrf" value="<?=h(csrf())?>"><button name="action" value="mail_probe">IMAP + SMTP testen</button></form><form method="post"><input type="hidden" name="csrf" value="<?=h(csrf())?>"><button class="approve" name="action" value="mail_loopback">Loopback senden & wiederfinden</button></form></div><?php endif;?>
</section>
<section>
  <h2>KiCom Slack</h2>
  <?php $ssx=kicomSlackStatus();$sm=kicomSlackManifest();?>
  <div class="grid">
    <div class="card"><strong>Workspace <?=h((string)$ssx['workspace'])?> · #<?=h((string)$ssx['channel_name'])?></strong><span class="big-state"><?=!empty($ssx['configured'])?'BEREIT':'APP-SECRETS FEHLEN'?></span><p class="muted">Team <code><?=h((string)$ssx['team_id'])?></code> · Channel <code><?=h((string)$ssx['channel_id'])?></code><br>Inbound nur <code>@KiCom</code>-Erwähnungen; Outbound ausschließlich nach <code>#kicom</code>.</p></div>
    <div class="card"><strong>Geschützte Slack-Secrets</strong><p class="muted">Bot Token und Signing Secret liegen ausschließlich unter <code>var/secrets</code> bzw. in der Server-Umgebung. Keine Ausgabe in Memory, GitHub oder Logs.</p><form method="post" autocomplete="off"><input type="hidden" name="csrf" value="<?=h(csrf())?>"><input type="hidden" name="action" value="slack_secret_save"><label>Bot Token<input type="password" name="slack_bot_token" required autocomplete="new-password" placeholder="xoxb-..."></label><label>Signing Secret<input type="password" name="slack_signing_secret" required autocomplete="new-password" placeholder="Signing Secret"></label><button class="approve">Geschützt speichern</button></form></div>
  </div>
  <p class="muted">Slack Events URL: <code><?=h((string)$ssx['events_url'])?></code> · Scopes: <code>app_mentions:read</code>, <code>chat:write</code> · Event: <code>app_mention</code>.</p>
  <?php if(!empty($ssx['configured'])):?><form method="post"><input type="hidden" name="csrf" value="<?=h(csrf())?>"><button class="approve" name="action" value="slack_probe">Slack-Bot testen</button></form><?php endif;?>
  <details><summary>Slack App Manifest</summary><pre><?=h((string)json_encode($sm,JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE))?></pre></details>
</section>
<section>
