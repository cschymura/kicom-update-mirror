 ...
 *
 * This bootstrap is deliberately narrower than the central artifact bus:
 * - configured HTTPS source family only;
 * - expected SHA-256 required before bytes are trusted;
 * - writes are confined to the existing /dev tree;
 * - replaced bytes and failed/new rollback bytes are retained in archives;
 * - no production self-update, recovery, kernel or secret-store authority.
 */
final class KiComDevArtifactImporter
{
    private const MAX_DOWNLOAD_BYTES = 16777216;
    private const MAX_TOTAL_BYTES = 12582912;
    ...
    public function __construct(string $devRoot, string $storageDir)
    {
        ...
        if (!is_string($real) || !is_dir($real)) {
            throw new RuntimeException('DEV_ARTIFACT_ROOT_INVALID');
        }
        $this->devRoot = rtrim($real, '/');
        $this->store = rtrim($storageDir, '/');
        if (!$this->ensureStore()) {
            throw new RuntimeException('DEV_ARTIFACT_STORAGE_UNAVAILABLE');
        }
    }
    ...
    public function status(): array
    {
        return [
            ...
            'production_self_update_bridge'=>false,
        ];
    }
    ...
    public function verifyRemote(string $url, string $expectedSha, string $format='auto'): array
    {
        ...
        try {
            $inspect = $this->inspectFile((string)$download['_file'], $format, false);
            ...
            $this->archiveDownloaded(
                (string)($download['_file'] ?? ''),
                'verified-only',
    ...
    public function installRemote(string $url, string $expectedSha, string $format='auto'): array
    {
        ...
        if (!empty($result['ok'])) {
            $result['source_host'] = (string)$download['source_host'];
            return $result;
        }
    ...
    public function installVerifiedBytes(string $bytes, string $expectedSha, string $format='auto', string $label='selftest'): array
    {
        ...
        if (strlen($bytes) > self::MAX_DOWNLOAD_BYTES) {
            return ['ok'=>false,'code'=>'DEV_ARTIFACT_TOO_LARGE'];
        }
        $file = $this->store.'/incoming/local-'.substr($expectedSha, 0, 16).'-'.bin2hex(random_bytes(3)).'.bin';
        if (@file_put_contents($file, $bytes, LOCK_EX) === false) {
            return ['ok'=>false,'code'=>'DEV_ARTIFACT_STAGE_WRITE_FAILED'];
        }
        @chmod($file, 0600);
    ...
    private function installVerifiedFile(string $file, string $sha, string $format, string $source): array
    {
        ...
        if (!$this->mkdir($stageRoot)
            || !$this->mkdir($archiveRoot.'/before')
            || !$this->mkdir($archiveRoot.'/rollback-new')) {
        ...
        foreach ($entries as $entry) {
            ...
            if (!$this->mkdir(dirname($stage)) || @file_put_contents($stage, $content, LOCK_EX) === false) {
                return ['ok'=>false,'code'=>'DEV_ARTIFACT_STAGE_WRITE_FAILED','path'=>$path];
            }
            @chmod($stage, 0600);

            $target = $this->devRoot.'/'.$path;
            if (is_link($target) || is_dir($target)) {
                return ['ok'=>false,'code'=>'DEV_ARTIFACT_TARGET_TYPE_FORBIDDEN','path'=>$path];
            }
            $had = is_file($target);
            if ($had) {
                $before = $archiveRoot.'/before/'.$path;
                if (!$this->mkdir(dirname($before)) || !@copy($target, $before)) {
                    return ['ok'=>false,'code'=>'DEV_ARTIFACT_ARCHIVE_FAILED','path'=>$path];
                }
                @chmod($before, 0600);
            }
            $changes[] = [
                'path'=>$path,
                'had_previous'=>$had,
                'before_sha256'=>$had ? (hash_file('sha256', $target) ?: null) : null,
        ...
        foreach ($changes as $change) {
            ...
            $stage = $stageRoot.'/'.$path;
            $target = $this->devRoot.'/'.$path;
            if (!$this->mkdir(dirname($target))) {
                $this->rollback($applied, $archiveRoot);
                return ['ok'=>false,'code'=>'DEV_ARTIFACT_TARGET_DIR_FAILED','path'=>$path];
            }
            $tmp = $target.'.kicom-artifact-'.bin2hex(random_bytes(4));
            if (!@copy($stage, $tmp)) {
                $this->rollback($applied, $archiveRoot);
                return ['ok'=>false,'code'=>'DEV_ARTIFACT_TARGET_WRITE_FAILED','path'=>$path];
            }
            @chmod($tmp, 0644);
            if (!@rename($tmp, $target)) {
                @rename($tmp, $archiveRoot.'/failed-temp-'.bin2hex(random_bytes(3)));
                $this->rollback($applied, $archiveRoot);
                return ['ok'=>false,'code'=>'DEV_ARTIFACT_TARGET_RENAME_FAILED','path'=>$path];
            }
            $applied[] = $change;
        }
    ...
    private function rollback(array $applied, string $archiveRoot): void
    {
        foreach (array_reverse($applied) as $change) {
            ...
            if (!empty($change['had_previous'])) {
                $before = $archiveRoot.'/before/'.$path;
                if (!is_file($before)) continue;
                $tmp = $target.'.rollback-'.bin2hex(random_bytes(3));
                if (@copy($before, $tmp)) {
                    @chmod($tmp, 0644);
                    if (!@rename($tmp, $target)) @rename($tmp, $archiveRoot.'/rollback-copy-failed-'.bin2hex(random_bytes(2)));
                }
    ...
    private function download(string $url, string $expectedSha): array
    {
        $expectedSha = strtolower(trim($expectedSha));
        if (!preg_match('/^[a-f0-9]{64}$/', $expectedSha)) {
            return ['ok'=>false,'code'=>'DEV_ARTIFACT_SHA_REQUIRED'];
        }
        $current = trim($url);
        for ($redirect=0; $redirect<=self::MAX_REDIRECTS; $redirect++) {
            ...
            $ctx = stream_context_create([
                'http'=>[
                    ...
                    'ignore_errors'=>true,
                    'timeout'=>25,
                    'user_agent'=>'KiCom-DEV-Artifact/1.1',
            ...
            if ($status !== 200) {
                fclose($fh);
                return ['ok'=>false,'code'=>'DEV_ARTIFACT_HTTP_STATUS','http_status'=>$status];
            }
            ...
            if ($out === false) {
                fclose($fh);
                return ['ok'=>false,'code'=>'DEV_ARTIFACT_STAGE_OPEN_FAILED'];
            }
            ...
            while (!feof($fh)) {
                ...
                if ($chunk === '') continue;
                $bytes += strlen($chunk);
                if ($bytes > self::MAX_DOWNLOAD_BYTES) { $ok=false; break; }
                hash_update($hash, $chunk);
                if (fwrite($out, $chunk) !== strlen($chunk)) { $ok=false; break; }
            }
            fclose($fh);
            fclose($out);
            @chmod($tmp, 0600);
            if (!$ok) {
                $this->archiveDownloaded($tmp, $bytes > self::MAX_DOWNLOAD_BYTES ? 'too-large' : 'download-failed', hash_file('sha256', $tmp) ?: '');
                return ['ok'=>false,'code'=>$bytes > self::MAX_DOWNLOAD_BYTES ? 'DEV_ARTIFACT_TOO_LARGE' : 'DEV_ARTIFACT_DOWNLOAD_FAILED'];
            }
            $actual = hash_final($hash);
            if (!hash_equals($expectedSha, $actual)) {
                $this->archiveDownloaded($tmp, 'sha-mismatch', $actual);
                return ['ok'=>false,'code'=>'DEV_ARTIFACT_SHA_MISMATCH','actual_sha256'=>$actual,'bytes'=>$bytes];
            }
            return [
                'ok'=>true,
                'code'=>'DEV_ARTIFACT_DOWNLOADED',
                'sha256'=>$actual,
                'bytes'=>$bytes,
                'source_host'=>(string)(parse_url($current, PHP_URL_HOST) ?: ''),
                '_file'=>$tmp,
            ];
        }
    ...
    /** @return array<string,mixed> */
    private function inspectFile(string $file, string $format, bool $includeContent): array
    {
        if (!is_file($file)) return ['ok'=>false,'code'=>'DEV_ARTIFACT_FILE_MISSING'];
        $head = @file_get_contents($file, false, null, 0, 4);
        if (!is_string($head)) return ['ok'=>false,'code'=>'DEV_ARTIFACT_READ_FAILED'];
        $format = strtolower(trim($format));
        if ($format === '' || $format === 'auto') $format = str_starts_with($head, 'PK') ? 'zip' : 'bundle';
    ...
    private function inspectBundle(string $file, bool $includeContent): array
    {
        ...
        if (!is_string($raw)) return ['ok'=>false,'code'=>'DEV_ARTIFACT_READ_FAILED'];
        try { $j=json_decode($raw, true, 64, JSON_THROW_ON_ERROR); }
        catch (Throwable $e) { return ['ok'=>false,'code'=>'DEV_ARTIFACT_BUNDLE_JSON_INVALID']; }
        if (!is_array($j) || ($j['schema'] ?? 0) !== 1 || ($j['type'] ?? '') !== 'kicom-dev-bundle' || ($j['target'] ?? '') !== 'dev') {
            return ['ok'=>false,'code'=>'DEV_ARTIFACT_BUNDLE_SCHEMA_INVALID'];
        }
        $files = is_array($j['files'] ?? null) ? $j['files'] : [];
        if (count($files) < 1 || count($files) > self::MAX_FILES) return ['ok'=>false,'code'=>'DEV_ARTIFACT_FILE_COUNT_INVALID'];
        $entries=[]; $seen=[]; $total=0;
        foreach ($files as $row) {
            if (!is_array($row)) return ['ok'=>false,'code'=>'DEV_ARTIFACT_ENTRY_INVALID'];
            $path = $this->safePath((string)($row['path'] ?? ''));
            if ($path === null || isset($seen[$path])) return ['ok'=>false,'code'=>'DEV_ARTIFACT_PATH_INVALID'];
            $seen[$path]=true;
            $content = base64_decode((string)($row['content_b64'] ?? ''), true);
            if (!is_string($content)) return ['ok'=>false,'code'=>'DEV_ARTIFACT_CONTENT_ENCODING_INVALID','path'=>$path];
            ...
            if (!preg_match('/^[a-f0-9]{64}$/', $sha) || !hash_equals($sha, hash('sha256', $content))) {
                return ['ok'=>false,'code'=>'DEV_ARTIFACT_FILE_SHA_MISMATCH','path'=>$path];
            }
            $v=$this->validateContent($path, $content);
            if (empty($v['ok'])) return $v+['path'=>$path];
            $total += strlen($content);
            if ($total > self::MAX_TOTAL_BYTES) return ['ok'=>false,'code'=>'DEV_ARTIFACT_UNPACKED_TOO_LARGE'];
    ...
    private function inspectZip(string $file, bool $includeContent): array
    {
        ...
        $z=new ZipArchive();
        if ($z->open($file) !== true) return ['ok'=>false,'code'=>'DEV_ARTIFACT_ZIP_OPEN_FAILED'];
        try {
            if ($z->numFiles < 1 || $z->numFiles > self::MAX_FILES+40) return ['ok'=>false,'code'=>'DEV_ARTIFACT_FILE_COUNT_INVALID'];
            $raw=[]; $allDevPrefix=true;
            for ($i=0; $i<$z->numFiles; $i++) {
                $stat=$z->statIndex($i);
                if (!is_array($stat)) return ['ok'=>false,'code'=>'DEV_ARTIFACT_ZIP_STAT_FAILED'];
                $name=(string)($stat['name'] ?? '');
                if ($name === '' || str_ends_with($name, '/')) continue;
                if (!str_starts_with($name, 'dev/')) $allDevPrefix=false;
            ...
            foreach ($raw as $row) {
                ...
                if ($allDevPrefix) $name=substr($name, 4);
                $path=$this->safePath($name);
                if ($path === null || isset($seen[$path])) return ['ok'=>false,'code'=>'DEV_ARTIFACT_PATH_INVALID','path'=>$name];
                $seen[$path]=true;
                $size=(int)$row['size'];
                if ($size < 0 || $size > self::MAX_FILE_BYTES) return ['ok'=>false,'code'=>'DEV_ARTIFACT_FILE_TOO_LARGE','path'=>$path];
                $ops=0; $attr=0;
                if ($z->getExternalAttributesIndex((int)$row['index'], $ops, $attr)) {
                    $mode=($attr >> 16) & 0170000;
                    if ($mode === 0120000) return ['ok'=>false,'code'=>'DEV_ARTIFACT_SYMLINK_FORBIDDEN','path'=>$path];
                }
                $content=$z->getFromIndex((int)$row['index']);
                if (!is_string($content) || strlen($content) !== $size) return ['ok'=>false,'code'=>'DEV_ARTIFACT_ZIP_READ_FAILED','path'=>$path];
                $v=$this->validateContent($path, $content);
                if (empty($v['ok'])) return $v+['path'=>$path];
                $total += $size;
                if ($total > self::MAX_TOTAL_BYTES) return ['ok'=>false,'code'=>'DEV_ARTIFACT_UNPACKED_TOO_LARGE'];
    ...
    private function validateContent(string $path, string $content): array
    {
        ...
        if ($ext === 'php') {
            ...
            catch (ParseError $e) { return ['ok'=>false,'code'=>'DEV_ARTIFACT_PHP_SYNTAX_INVALID']; }
        } elseif ($ext === 'json') {
            try { json_decode($content, true, 128, JSON_THROW_ON_ERROR); }
            catch (Throwable $e) { return ['ok'=>false,'code'=>'DEV_ARTIFACT_JSON_INVALID']; }
        } elseif ($base === '.htaccess') {
            if (strlen($content) > 4096 || preg_match('/\b(SetHandler|AddHandler|auto_prepend_file|auto_append_file)\b/i', $content)) {
                return ['ok'=>false,'code'=>'DEV_ARTIFACT_HTACCESS_FORBIDDEN'];
            }
        }
    ...
    {
        $path=trim($path);
        if ($path === '' || strlen($path) > 220 || str_contains($path, "\0") || str_contains($path, '\\') || str_starts_with($path, '/') || preg_match('/^[A-Za-z]:/', $path)) return null;
        foreach (explode('/', $path) as $part) {
            if ($part === '' || $part === '.' || $part === '..' || strlen($part) > 120) return null;
        }
    ...
    private function validateUrl(string $url): array
    {
        ...
        $p=parse_url($url);
        if (!is_array($p) || strtolower((string)($p['scheme'] ?? '')) !== 'https' || empty($p['host']) || isset($p['user']) || isset($p['pass'])) {
            return ['ok'=>false,'code'=>'DEV_ARTIFACT_HTTPS_REQUIRED'];
        }
        if (isset($p['port']) && (int)$p['port'] !== 443) return ['ok'=>false,'code'=>'DEV_ARTIFACT_PORT_FORBIDDEN'];
        $host=strtolower(rtrim((string)$p['host'], '.'));
        if (filter_var($host, FILTER_VALIDATE_IP)) return ['ok'=>false,'code'=>'DEV_ARTIFACT_IP_LITERAL_FORBIDDEN'];
    ...
    private function httpStatus(array $headers): int
    {
        ...
        foreach ($headers as $h) {
            if (preg_match('#^HTTP/\S+\s+(\d{3})#i', (string)$h, $m)) $status=(int)$m[1];
        }
        return $status;
    }
    ...
    private function headerValue(array $headers, string $name): ?string
    {
        foreach (array_reverse($headers) as $h) {
            if (stripos((string)$h, $name.':') === 0) return trim(substr((string)$h, strlen($name)+1));
        }
        return null;
    }
    ...
    private function resolveRedirect(string $base, string $location): string
    {
        ...
        $port='';
        if (isset($p['port'])) $port=':'.(string)(int)$p['port'];
        $origin='https://'.(string)$p['host'].$port;
    ...
    {
        foreach ([$this->store,$this->store.'/incoming',$this->store.'/staging',$this->store.'/archive'] as $dir) {
            if (!$this->mkdir($dir)) return false;
        }
        $deny=$this->store.'/.htaccess';
    ...
    private function archiveDownloaded(string $file, string $reason, string $sha): void
    {
        ...
        $cleanSha=preg_replace('/[^a-f0-9]/', '', strtolower($sha)) ?? '';
        $dir=$this->store.'/archive/'.gmdate('Ymd\THis\Z').'-'.$reason.'-'.substr($cleanSha,0,16).'-'.bin2hex(random_bytes(2));
    ...
    private function sanitizeSource(string $source): string
    {
        ...
        if (is_array($p) && !empty($p['host'])) {
            return 'https://'.strtolower((string)$p['host']).(string)($p['path'] ?? '');
        }
