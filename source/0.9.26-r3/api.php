...
/* 0.9.8 high-throughput raw update upload. Session auth rotates once for the whole body. */
if(in_array($queryOp,['AUTONOMY_UPDATE_UPLOAD','CHAT_UPDATE_RAW'],true)){
    $chatRaw=$queryOp==='CHAT_UPDATE_RAW';$opName=$chatRaw?'chat_update_raw':'autonomy_update_upload';
    $sid=(string)($_SERVER['HTTP_X_KICOM_SESSION_ID']??($_GET['session_id']??''));$tok=(string)($_SERVER['HTTP_X_KICOM_SESSION_TOKEN']??($_GET['token']??''));$ss=kicomAutonomySessionConsume(strtolower($sid),strtolower($tok));if(empty($ss['ok']))apiOut([KCL_PROTOCOL,'ERROR autonomy_update_upload','FACT code='.apiKclString((string)($ss['code']??'SESSION_REJECTED')),'END'],401);$len=(int)($_SERVER['CONTENT_LENGTH']??0);if($len<=0||$len>KICOM_MAX_SELF_UPDATE_ZIP_BYTES)apiOut([KCL_PROTOCOL,'ERROR autonomy_update_upload','FACT code="PACKAGE_SIZE_INVALID"','FACT next_token='.apiKclString((string)$ss['next_token'],100),'END'],413);$raw=file_get_contents('php://input');if($raw===false||strlen($raw)!==$len)apiOut([KCL_PROTOCOL,'ERROR autonomy_update_upload','FACT code="BODY_READ_FAILED"','FACT next_token='.apiKclString((string)$ss['next_token'],100),'END'],400);$name=basename((string)($_SERVER['HTTP_X_KICOM_FILENAME']??'KiCom-autonomy-upload.zip'));if(strtolower(pathinfo($name,PATHINFO_EXTENSION))!=='zip')$name.='.zip';$tmp=kicomTempDir().'/autonomy-raw-'.substr(hash('sha256',$raw),0,24).'.zip';if(@file_put_contents($tmp,$raw,LOCK_EX)===false)apiOut([KCL_PROTOCOL,'ERROR autonomy_update_upload','FACT code="TEMP_WRITE_FAILED"','FACT next_token='.apiKclString((string)$ss['next_token'],100),'END'],500);@chmod($tmp,0600);try{$r=kicomReceiveSelfUpdatePackage($tmp,$name,($chatRaw?'chatgpt:direct':'autonomy:raw-post'),true);if(!empty($r['ok'])&&($r['code']??'')==='STAGED_DECISION_REQUIRED'){ $policy=kicomAutonomyPolicy();$p=kicomSelfUpdatePending();if(is_array($p)&&(!empty($policy['internal_self_update'])||(($r['risk_class']??($r['staged']['risk_class']??''))==='yellow'&&!empty($policy['auto_yellow_from_session']))))$r=kicomApplySelfUpdate($p)+['code'=>'AUTO_INSTALLED_INTERNAL_SESSION'];}}finally{@unlink($tmp);}$lines=[KCL_PROTOCOL,(!empty($r['ok'])?'OK':'ERROR').' '.$opName,'FACT code='.apiKclString((string)($r['code']??'UNKNOWN')),'FACT session_id='.apiKclString((string)$ss['session_id']),'FACT next_token='.apiKclString((string)$ss['next_token'],100)];foreach(['to_version','risk_class','history_id'] as $k)if(isset($r[$k])&&is_scalar($r[$k]))$lines[]='FACT '.$k.'='.apiKclString((string)$r[$k]);$lines[]='END';apiOut($lines,!empty($r['ok'])?200:422);
}
...
case 'AUTONOMY_CODEBOOK_BEGIN':
    $r=kicomCodebookBegin((string)($data['session_id']??''),(string)($data['token']??''));$lines=[KCL_PROTOCOL,(!empty($r['ok'])?'OK':'ERROR').' autonomy_codebook_begin','FACT request_id='.apiKclString($rid),'FACT code='.apiKclString((string)($r['code']??'UNKNOWN'))];foreach(['context_id','expires_in','codebook','codebook_sha256','gzip_available'] as $k)if(isset($r[$k]))$lines[]='FACT '.$k.'='.apiKclString(is_bool($r[$k])?($r[$k]?'true':'false'):(string)$r[$k]);$lines[]='END';apiOut($lines,!empty($r['ok'])?200:401);
case 'AUTONOMY_CODEBOOK_VALIDATE':
    $r=kicomCodebookRun((string)($data['session_id']??''),(string)($data['token']??''),(string)($data['context_id']??''),(string)($data['codec']??'r'),(string)($data['data']??''),false);$lines=[KCL_PROTOCOL,(!empty($r['ok'])?'OK':'ERROR').' autonomy_codebook_validate','FACT request_id='.apiKclString($rid),'FACT code='.apiKclString((string)($r['code']??'UNKNOWN'))];foreach(['operations','wire_bytes','program_bytes','expanded_json_bytes','codebook_sha256'] as $k)if(isset($r[$k]))$lines[]='FACT '.$k.'='.apiKclString((string)$r[$k]);$lines[]='END';apiOut($lines,!empty($r['ok'])?200:422);
case 'AUTONOMY_CODEBOOK_EXEC':
    $r=kicomCodebookRun((string)($data['session_id']??''),(string)($data['token']??''),(string)($data['context_id']??''),(string)($data['codec']??'r'),(string)($data['data']??''),true);$lines=[KCL_PROTOCOL,(!empty($r['ok'])?'OK':'ERROR').' autonomy_codebook_exec','FACT request_id='.apiKclString($rid),'FACT code='.apiKclString((string)($r['code']??'UNKNOWN'))];foreach(['session_id','next_token','session_expires_at','session_idle_expires_at','operations','wire_bytes','program_bytes','expanded_json_bytes','codebook_sha256'] as $k)if(isset($r[$k]))$lines[]='FACT '.$k.'='.apiKclString((string)$r[$k],100);foreach(($r['results']??[]) as $i=>$x)if(is_array($x))$lines[]='OP #'.($i+1).' resource='.apiKclString((string)($x['resource']??'')).' ok='.(!empty($x['ok'])?'true':'false').' code='.apiKclString((string)($x['code']??'OK'));$lines[]='END';apiOut($lines,!empty($r['ok'])?200:422);

case 'AUTONOMY_BATCH':
    $sid=strtolower((string)($data['session_id']??''));$tok=strtolower((string)($data['token']??''));$ss=kicomAutonomySessionConsume($sid,$tok);if(empty($ss['ok']))apiOut([KCL_PROTOCOL,'ERROR autonomy_batch','FACT request_id='.apiKclString($rid),'FACT code='.apiKclString((string)($ss['code']??'SESSION_REJECTED')),'END'],401);$ops=$data['operations']??null;if(!is_array($ops)||count($ops)<1||count($ops)>32)apiOut([KCL_PROTOCOL,'ERROR autonomy_batch','FACT request_id='.apiKclString($rid),'FACT code="BATCH_INVALID"','FACT next_token='.apiKclString((string)$ss['next_token'],100),'END'],400);$lines=[KCL_PROTOCOL,'OK autonomy_batch','FACT request_id='.apiKclString($rid),'FACT session_id='.apiKclString((string)$ss['session_id']),'FACT next_token='.apiKclString((string)$ss['next_token'],100),'FACT operations='.count($ops)];$okAll=true;foreach($ops as $i=>$o){if(!is_array($o)){$r=['ok'=>false,'code'=>'OP_INVALID'];}else{$kind=strtolower((string)($o['kind']??''));$find=kicomDecodeBase64Url((string)($o['find']??''),true);$replace=kicomDecodeBase64Url((string)($o['replace']??''),true);if($find===null||$replace===null)$r=['ok'=>false,'code'=>'PATCH_ENCODING_INVALID'];elseif($kind==='memory_patch')$r=kicomAutonomyMemoryPatchDirect((string)($o['resource']??''),(string)($o['base_sha256']??''),$find,$replace);elseif($kind==='workspace_patch')$r=kicomAutonomyWorkspacePatchDirect((string)($o['path']??''),(string)($o['base_sha256']??''),$find,$replace);elseif($kind==='build_patch')$r=kicomFastBuildPatch((string)$ss['session_id'],(string)($o['build_id']??''),(string)($o['path']??''),(string)($o['base_sha256']??''),$find,$replace);else$r=['ok'=>false,'code'=>'OP_KIND_UNSUPPORTED'];}$okAll=$okAll&&!empty($r['ok']);$lines[]='OP #'.($i+1).' ok='.(!empty($r['ok'])?'true':'false').' code='.apiKclString((string)($r['code']??'OK')).(isset($r['sha256'])?' sha256='.apiKclString((string)$r['sha256']):'');}$lines[]='FACT all_ok='.($okAll?'true':'false');$lines[]='END';apiOut($lines,$okAll?200:207);
case 'GOAL_PROPOSE':
