...
The original `admin.php` was extracted fresh from the verified ZIP in an isolated workspace. The **corrected** active-upgrade route patch applied using **`patch --fuzz=0 -p1`**. The exact committed DEV95 route patch was first found to require fuzzy matching against the original SHA-verified 0.9.37 archive; it was then corrected to a canonical six-context-line unified diff, and the fixed file was committed at SHA **72b53e08aba92fbcfb11bf45a727fb3a5b8849ce**. The corrected original-native route was retested with `patch --fuzz=0` against freshly extracted original admin.php, then the original menu received its own one-line patch with `--fuzz=0`. The staged admin.php passed `php -l`; exactly one new navigation target, exactly one original OAuthAuthorizeHttp::handle reference, and two original owner-activation links were observed. **No original live admin, token or database changed.**
