...
$noResource=str_replace('&resource='.rawurlencode($resource),'',$refresh);
$r=KiComEngramOAuthGrantForm::parse($noResource,$client);
ok($r['kind']==='refresh_token'&&$r['arguments']===['refresh_token'=>$token],'refresh without optional resource uses server-pinned audience');
...
['wrong resource',str_replace(rawurlencode($resource),rawurlencode('https://other.example/api'),$refresh)],
['code missing resource',str_replace('&resource='.rawurlencode($resource),'',$code)],
];
