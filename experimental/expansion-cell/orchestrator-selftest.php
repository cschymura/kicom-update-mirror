...
try{
 ...
 foreach(['ExpansionProtocol.php','CellNode.php','CellLiving.php'] as $f) chk(copy(__DIR__.'/'.$f,$src.'/'.$f),'copy source '.$f);
 ...
 $seed=require $pkg['directory'].'/bootstrap.config.php';$child=new KiComExpansionCellNode($pkg['directory'].'/var');$init=$child->initialize($seed);chk(!empty($init['ok']),'child init');chk(!empty($init['living_ready']),'living ready at birth');$born=$child->status();chk(!empty($born['living_ready']),'status proves living ready before enrollment');
 $done=$o->activatePrepared($prep,new FakeTransport($child));chk(!empty($done['ok'])&&($done['code']??'')==='EXPANSION_ACTIVE','activation+tick');$s=$child->status();chk(($s['state']??'')==='active','child active');chk(!empty($s['living_ready']),'child still living ready');chk(count($reg->cells())===1,'registry cell');
